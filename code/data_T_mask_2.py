'''
供 Transformer 的使用
1. 使用动态mask机制随机mask掉doc中的某个entity
2. 如果dev、test中的实体在train中出现过，则不再 MASK，否则执行mask操作

此文件是提供给 train_bert_rdrop_t_cs.py 使用的
'''
...
from utils import get_cuda,get_all_entity,tokenizer_entity,get_labelid2name
...
dev_entity2id = get_all_entity("../data/dev.json") # 切记，这里必须是dev.json，不能贪图方便使用dev_200.json
test_entity2id = get_all_entity("../data/test.json")
    ...
    def __init__(self, src_file, save_file, word2id, ner2id, rel2id,
        ...
        if os.path.exists(save_file):
            ...
            for i, doc in enumerate(ori_data):
                ...
                id2entity_name_doc = defaultdict(list) # 当前这个doc中，实体id到名称的映射
                id2entity_type_doc = {} 
                for idx,entity in enumerate(entity_list,1):
                    for mention in entity:
                        name = mention['name']
                        id2entity_name_doc[idx].append(name)
                        id2entity_type_doc[idx] = mention['type']
                ...
                self.data.append(
                    ...
                    'entity2mention': entity2mention,                    
                    'overlap': new_overlap,                    
                    'id2entity_name_doc': id2entity_name_doc,  # 当前doc的entity id 到name的映射
                    'id2entity_type_doc': id2entity_type_doc,  # 当前doc的entity id 到实体类型的映射
                    'bert_subwords':bert_subwords # 用于后面调试输出
                    }
                )
...
class DGLREDataloader(DataLoader):
    ...
    def __init__(self, dataset, batch_size, shuffle=False, h_t_limit_per_batch=300, h_t_limit=1722, relation_num=97,max_length=512, negativa_alpha=0.0, dataset_type='train',pos_lsr = 0.9,neg_lsr = 0.9,mask_prob=0.035):
        ...
        self.entity_type_length2token = tokenizer_entity() # 获取实体类型+长度 => token id的映射
    ...
    def __iter__(self):
        ...
        for idx, minibatch in enumerate(self.batches):
            ...
            ht_pairs_entity_type_id = torch.zeros(cur_bsz, self.h_t_limit, dtype=torch.long).cpu() # 初始值为0
            ...
            for i, example in enumerate(minibatch):
                title, entities, labels, na_triple, word_id, pos_id, ner_id, mention_id, entity2mention, bert_tokens,id2entity_name_doc,id2entity_type_doc = \
                    ...
                    example['word_id'], example['pos_id'], example['ner_id'], example['mention_id'], example[
                        'entity2mention'], example['bert_subwords'],example['id2entity_name_doc'],example['id2entity_type_doc']
                ...
                # 随机挑选出一个实体，然后将其id替换成 [MASK]的id
                cur_entity_num = max(pos_id) # 获取当前doc的实体个数
                
                # aaaa = self.tokenizer.convert_ids_to_tokens(word_id)
                # tail_idx = aaaa.index("[SEP]")
                # print("原始文本："," ".join(aaaa[:tail_idx]))

                # TODO: 每次只mask一个实体？ 还是可以多个实体一起 MASK?
                a = random.uniform(0,1)
                if a < self.mask_prob: 
                    flag = 1
                    candidate = [i for i in range(1,cur_entity_num+1)] # 待选择的区间
                    while flag and len(candidate):
                        mask_entity_id = random.choice(candidate) # 随机选择一个
                        # 获取当前这个位置的实体的类型
                        select_mask_entity_type = id2entity_type_doc[mask_entity_id]

                        # 获取当前这个位置的实体的名称，查看其是否在dev中出现过，如果出现过，则换一个实体 MASK
                        select_mask_entity_name = id2entity_name_doc[mask_entity_id] # 得到被mask的这个实体的名称
                        flag = 0
                        for name in select_mask_entity_name: 
                            # 如果当前这个实体在dev或者在test 中都出现过，那么就不要mask了
                            if name in dev_entity2id.keys() or name in test_entity2id.keys(): 
                                flag = 1
                                candidate.remove(mask_entity_id) # 从待选择的区间中删除掉这个值
                                break
                    if len(candidate): # 如果还有元素存在，说明可以继续
                        # 从 entity_type_length2_token 中随机挑一个作为 MASK 的值
                        choice_range = self.entity_type_length2token[select_mask_entity_type+"_"+str(len(word_id[pos_id==mask_entity_id]))]
                        if len(choice_range) > 0: # 如果有的选，否则直接用 [MASK] = 103 替代
                            replace_token_id = random.choice(choice_range)                            
                            word_id[pos_id==mask_entity_id] = replace_token_id 
                        else:
                            word_id[pos_id==mask_entity_id] = 103 # 103 是 [MASK] 的token id
                
                        # aaaa = self.tokenizer.convert_ids_to_tokens(word_id)
                        # tail_idx = aaaa.index("[SEP]")
                        # print("修改后的文本："+" ".join(aaaa[:tail_idx]))
                ...
                if self.dataset_type == 'train':
                    ...
                    for j, (h_idx, t_idx) in enumerate(na_triple[:lower_bound], len(train_triple)):
                        ...
                        #     ht_pair_distance[i, j] = int(self.dis2idx[delta_dis]) + self.dis_size // 2
                        # 因为是负样本，所以这里的 r=0
                        relation_multi_label[i, j, 0] = self.neg_lsr
