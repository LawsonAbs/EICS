'''
使用 length_mask 控制每个doc中 attention的范围
'''
import wandb
import time
...
from config import *
from data_T_distance import DGLREDataset, DGLREDataloader, BERTDGLREDataset  # 这是从data.py 中引入几个类
from models.BERT_T_distance import BERT_T
from evaluate_T_constraint_distance import eval_cons
...
def train(opt):
    ...
    if opt.use_model == 'bert':
        ...
        train_loader = DGLREDataloader(train_set, batch_size=opt.batch_size, shuffle=True,
                                       negativa_alpha=opt.negativa_alpha, window = opt.window )
        dev_loader = DGLREDataloader(dev_set, batch_size=opt.test_batch_size, dataset_type='dev', window = opt.window )
...
if __name__ == '__main__':
    ...
    wandb.init(project="train_bert_T_distance", entity="lawsonabs",config=opt)
