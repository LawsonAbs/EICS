...
使用bert中的低层embedding，主要是获取预训练模型的常识知识。
'''
...
from data_cr import  DGLREDataloader, BERTDGLREDataset  # 这是从data.py 中引入几个类
...
from models.BERT_rdrop_t_cs_cr import  BERT_T
from evaluate_bert_rdrop_t_cs_cr import eval
...
def train(opt):
    ...
    if opt.use_model == 'bert':
        ...
        bilinear_ids = list(map(id,bert_T.W_bilinear.parameters()))
        ...
        # 返回对象是一个filter
        linar_params = filter(lambda p: p.requires_grad and id(p) not in bert_param_ids+transformer_param_ids+bilinear_ids, bert_T.parameters())

        optimizer = optim.AdamW([
            {'params': bert_T.bert.parameters(), 'lr': opt.lr_bert},
            {'params': bert_T.bert_encoder.parameters(), 'lr': opt.lr_tran},
            # {'params': bert_T.word_embeddings.parameters(), 'lr': opt.lr_word_embedding}, 
            {'params': linar_params, 'weight_decay': opt.weight_decay,'lr': opt.lr},
            {'params': bert_T.W_bilinear.parameters(), 'weight_decay': opt.weight_decay,'lr':opt.lr},
        ], lr=lr) # 默认 lr
    ...
    for epoch in range(start_epoch, opt.epoch + 1):
        ...
        for ii, d in enumerate(train_loader): # 送入模型的是一个batch 的数据
            ...
            # 验证参数是否更新
            # for param in bert_T.named_parameters():
            #     name,value = param
            #     # print(name)
            #     if "bilinear" in name:
            #         print("bilinear",end=",")
            #         print(f"value={value[0:20,0]}")
            ...
            predictions_bert_1,batch_cr_scores_1 = bert_T(words=d['context_idxs'],
                                ...
                                entity_coref_mask = d['entity_coref_mask']
            ...
            # 第二次使用 bert 
            predictions_bert_2,batch_cr_scores_2 = bert_T(words=d['context_idxs'],
                                src_lengths=d['context_word_length'],
                                mask=d['context_word_mask'],
                                entity_type=d['context_ner'],
                                entity_id=d['context_pos'],
            ...
            # loss_bert_1 = torch.sum(BCE(predictions_bert_1, relation_multi_label) * relation_mask.unsqueeze(2)) / (
            #         opt.relation_num * torch.sum(relation_mask)) 

            # loss_bert_2 = torch.sum(BCE(predictions_bert_2, relation_multi_label) * relation_mask.unsqueeze(2)) / (
            #         opt.relation_num * torch.sum(relation_mask)) 
        ...
        if acc_PA.get() >= 0.85 and acc_PA.get() < 0.92:
            opt.dev_period = 3
        elif acc_PA.get() >= 0.92:
            opt.dev_period = 1
...
if __name__ == '__main__':
    ...
    wandb.init(project="train_bert_rdrop_t_cs_cr", entity="lawsonabs")
