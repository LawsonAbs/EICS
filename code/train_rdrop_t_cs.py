...
【消融实验】
1.去除bert
2.仅使用bert训练好的 embedding，主要是获取预训练模型的常识知识，传入到2层transformers 框架，接着执行分类任务。
...
def train(opt):
    ...
    if opt.use_model == 'bert':
        ...
        word_embeddings_ids = list(map(id,bert_T.word_embeddings.parameters()))
        # TODO: 验证一下逻辑是否正确
        # 对 model.parameters() 参数进行一个过滤操作
        # 这个操作最后得到的结果不应该是空的吗？ => 不是，会对 线性层等参数进行过滤
        # 返回对象是一个filter
        linar_params = filter(lambda p: p.requires_grad and id(p) not in bert_param_ids+transformer_param_ids+word_embeddings_ids, bert_T.parameters())
    ...
    for epoch in range(start_epoch, opt.epoch + 1):
        ...
        if acc_PA.get() >= 0.1 and acc_PA.get() < 0.3:
            opt.dev_period = 3
        elif acc_PA.get() >= 0.3:
            opt.dev_period = 1
        ...
        if epoch % opt.dev_period == 0 and acc_PA.get() >= 0.1:
            logging('-' * 89)
...
if __name__ == '__main__':
    ...
    wandb.init(project="train_rdrop_t_cs", entity="lawsonabs")
