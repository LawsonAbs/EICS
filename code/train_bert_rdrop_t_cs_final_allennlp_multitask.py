...
3.针对allenNLP 修改后的指代数据使用的方法。
4.使用多任务学习的方式，来辨别文档中的指示代词。
'''
...
from models.BERT_rdrop_t_cs_word_embedding_combine_allennlp_multitask import  BERT_T_CR_MT
from evaluate_bert_rdrop_t_cs_allennlp_mt import eval
...
def train(opt):
    ...
    for epoch in range(start_epoch, opt.epoch + 1):
        ...
        for ii, d in enumerate(train_loader): # 送入模型的是一个batch 的数据
            ...
            predictions_bert_1,batch_loss_cr_1 = bert_T(words=d['context_idxs'],
            ...
            # 第二次使用 bert 
            predictions_bert_2,batch_loss_cr_2= bert_T(words=d['context_idxs'],
                                src_lengths=d['context_word_length'],
                                mask=d['context_word_mask'],
                                entity_type=d['context_ner'],
                                entity_id=d['context_pos'],
            ...
            loss = ((loss_bert_1 + loss_bert_2)/2  + kl_1) #+ 0.08 * batch_loss_cr_1
...
if __name__ == '__main__':
    ...
    wandb.init(project="train_bert_rdrop_t_cs_final_allennlp", entity="lawsonabs")
