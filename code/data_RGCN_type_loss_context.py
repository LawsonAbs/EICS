    ...
    def __init__(self, src_file, save_file, word2id, ner2id, rel2id,
        ...
        if os.path.exists(save_file):
            ...
            for i, doc in enumerate(ori_data):
                ...
                entity_id2token_position =defaultdict(list) # 用于记录一个实体的各个mention在被tokenizer 之后， 得到的位置
                ...
                for idx, vertex in enumerate(entity_list, 1): # 对所有的entity遍历
                    for v in vertex: # vertex 代表的是这个entity的所有 mention 集合
                        ...
                        already_exist.add((pos0, pos1))
                        # 将实体所在的 token 位置放到其中
                        entity_id2token_position[idx-1].append((pos0,pos1)) # 左闭右开
                replace_i = 0
                # TODO 功能存疑
                ...
                self.data.append(
                    ...
                    'entity_id2token_position':entity_id2token_position
                    }
                )
...
class DGLREDataloader(DataLoader):
    ...
    # window : 用于控制实体对的上下文信息的上下文长度。注意这里的window是实体对的左右各侧的长度，但是不包括实体自身的长度
    def __init__(self, dataset, batch_size, shuffle=False, h_t_limit_per_batch=300, h_t_limit=1722, relation_num=97,max_length=512, negativa_alpha=0.0, dataset_type='train',pos_lsr = 0.9,neg_lsr = 0.9,window =10):
    ...
    def __iter__(self):
        ...
        for idx, minibatch in enumerate(self.batches):
            ...
            # 每篇doc中的每个实体对都需要一个mask值
            entity_pair_context_mask = torch.zeros(self.batch_size,self.h_t_limit,self.max_length,dtype=torch.float32)
            max_h_t_cnt = 0
            ...
            for i, example in enumerate(minibatch):
                title, entities, labels, na_triple, word_id, pos_id, ner_id, mention_id, entity2mention, entity_id ,entity_id2token_position = \
                    ...
                    example['word_id'], example['pos_id'], example['ner_id'], example['mention_id'], example[
                        'entity2mention'],example['entity_id'],example['entity_id2token_position']
                cur_doc_token_length = sum(word_id!=0) # 当前这篇doc 在tokenizer 之后得到的总的非0的token长度。含CLS和SEP
                ...
                if self.dataset_type == 'train':
                    ...
                    for j, (h_idx, t_idx) in enumerate(train_triple):
                        ...
                        # 获取第h_idx ，t_idx 实体在tokenizer之后的位置，然后对这些位置相邻的window*2个token进行赋值成mask
                        h_position = entity_id2token_position[h_idx] 
                        t_position = entity_id2token_position[t_idx] 
                        
                        # 要去掉实体自己的位置
                        for pos in h_position:
                            start,end = pos
                            left = max(1,start-self.window) # 不带 cls 玩儿
                            right = min(end+self.window,cur_doc_token_length-1) # 这里的最大值应该是当前 token后 长度的最大值，而不是默认的512
                            # 只对实体两侧的token赋值                            
                            entity_pair_context_mask[i][j][left:start] = 1
                            entity_pair_context_mask[i][j][end:right] = 1                             
                        # 为了避免交叉，这里赋值成0
                        for pos in h_position:
                            start,end = pos
                            entity_pair_context_mask[i][j][start:end] = 0 

                        
                        for pos in t_position:
                            start,end = pos
                            left = max(1,start-self.window)
                            right = min(end+self.window,cur_doc_token_length-1)
                            entity_pair_context_mask[i][j][left:start] = 1
                            entity_pair_context_mask[i][j][end:right] = 1 
                            entity_pair_context_mask[i][j][start:end] = 0 # 为了避免交叉，这里赋值成0
                        # 为了避免交叉，这里赋值成0
                        for pos in t_position:
                            start,end = pos
                            entity_pair_context_mask[i][j][start:end] = 0 
                    ...
                    for j, (h_idx, t_idx) in enumerate(na_triple[:lower_bound], len(train_triple)):
                        ...
                        for pos in h_position:
                            start,end = pos
                            left = max(1,start-self.window)
                            right = min(end+self.window,cur_doc_token_length)
                            entity_pair_context_mask[i][j][left:start] = 1 # start 不赋值
                            entity_pair_context_mask[i][j][end:right] = 1 # right 不赋值
                        # 为了避免交叉，这里赋值成0
                        for pos in h_position:
                            start,end = pos
                            entity_pair_context_mask[i][j][start:end] = 0 
            ...
            yield {'context_idxs': get_cuda(context_word_ids[:cur_bsz, :batch_max_length].contiguous()),
                   ...
                   'entity_pair_context_mask':get_cuda(entity_pair_context_mask[:cur_bsz,:max_h_t_cnt,:]) # 返回最后的实体对该mask的信息
                   }