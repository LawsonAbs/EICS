...
from config import *
from data_T import DGLREDataset, DGLREDataloader, BERTDGLREDataset  # 这是从data.py 中引入几个类
from models.BERT_T_similarity import BERT_T
from evaluate_T_constraint_similarity import eval_cons
...
def train(opt):
    ...
    for epoch in range(start_epoch, opt.epoch + 1):
        ...
        for ii, d in enumerate(train_loader): # 送入模型的是一个batch 的数据
            ...
            predictions,sim_loss = model(words=d['context_idxs'],
            ...
            if global_step % log_step == 0:
                ...
                avg_step_sim_loss = total_sim_loss / log_step

                elapsed = time.time() - start_time
                logging(
                    '| epoch {:2d} | step {:4d} |  ms/b {:5.2f} | train loss {:5.3f} | sim loss {:5.3f} | NA acc: {:4.2f} | PA acc: {:4.2f}  | tot acc: {:4.2f} '.format(
                        epoch, global_step, elapsed * 1000 / log_step, cur_loss * 1000,avg_step_sim_loss, acc_NA.get(), acc_PA.get(),
                        acc_total.get()))
                wandb.log({"loss": cur_loss,"sim_loss":sim_loss }, step=global_step)
                total_loss = 0
                total_sim_loss = 0
                start_time = time.time()
...
if __name__ == '__main__':
    ...
    wandb.init(project="train_bert_T_similarity", entity="lawsonabs",config=opt)
