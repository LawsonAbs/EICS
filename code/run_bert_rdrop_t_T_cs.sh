...
if true; then
  ...
  mask_prob=0
  alpha=0
  pair_interaction_layers=1
  fig_result_dir=logs/${date}/${model_name}_${time}
  if [ ! -d ${fig_result_dir} ]; then # 用于判断是否存在文件夹
  mkdir -p ${fig_result_dir}  
  fi  
  nohup python -u train_bert_rdrop_t_T_cs.py \
    --notes "输入前attention前使用layernorm+使用2层transformer对实体对进行交互" \
    ...
    --mask_prob ${mask_prob} \
    --alpha ${alpha} \
    --pair_interaction_layers ${pair_interaction_layers} \
