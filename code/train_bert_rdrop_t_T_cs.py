...
from models.BERT_rdrop_t_T_cs import  BERT_T
from evaluate_bert_rdrop_t_T_cs import eval
...
def train(opt):
    ...
    if opt.use_model == 'bert':
        ...
        encoder_entity_ids = list(map(id, bert_T.encoder_entity.parameters())) 
        encoder_entity_pair_ids = list(map(id, bert_T.encoder_entity_pair.parameters()))  
        # TODO: 验证一下逻辑是否正确
        # 对 model.parameters() 参数进行一个过滤操作
        # 这个操作最后得到的结果不应该是空的吗？ => 不是，会对 线性层等参数进行过滤
        # 返回对象是一个filter
        linar_params = filter(lambda p: p.requires_grad and id(p) not in bert_param_ids+encoder_entity_ids+encoder_entity_pair_ids, bert_T.parameters())

        optimizer = optim.AdamW([
            {'params': bert_T.bert.parameters(), 'lr': opt.lr_bert},
            {'params': bert_T.encoder_entity.parameters(), 'lr': opt.lr_tran},
            {'params': bert_T.encoder_entity_pair.parameters(), 'lr': opt.lr_tran},
    ...
    for epoch in range(start_epoch, opt.epoch + 1):
        ...
        if acc_PA.get() >= 0.5 and acc_PA.get() < 0.85:
            opt.dev_period = 3
        elif acc_PA.get() >= 0.85:
            opt.dev_period = 1
...
if __name__ == '__main__':
    ...
    wandb.init(project="train_bert_rdrop_t_cs", entity="lawsonabs")
