'''
在预处理数据的时候，添加对Coreference 的处理，主要包括：
1.建立一个entity -> 指代 位置的mask矩阵
'''
...
from utils import get_cuda,get_all_entity,get_labelid2name

label_id2name = get_labelid2name()
IGNORE_INDEX = -100
...
class BERTDGLREDataset(IterableDataset):
    def __init__(self, src_file, save_file, word2id, ner2id, rel2id,
        ...
        if os.path.exists(save_file):
            ...
            if "bert-base-uncased" in opt.bert_path or "bert-base-cased" in opt.bert_path:
                bert = Bert(BertModel, 'bert-base-uncased', opt.bert_path)
            else:
                bert = Bert(RobertaModel, 'roberta-large', opt.bert_path)
            ...
            for i, doc in enumerate(ori_data):
                ...
                # 添加 coreference 信息
                title, entity_list, labels, sentences,coreference = \
                    doc['title'], doc['vertexSet'], doc.get('labels', []), doc['sents'],doc.get('pronoun',[])                
                ...
                for mentions in entity_list:
                    ...
                    cur_entity_id.append(entity2id.get(name,-1))
                ...
                entity_id2pos = {} # 存储每个实体id在文中的开始位置
                entity_id2type = {} # 存储每个实体id对应的类型信息
                # 找出每个实体所在的位置信息，用最小的 position 表示
                # step2. 遍历每个entity
                for j in range(len(entity_list)):                     
                    for k in range(len(entity_list[j])): # 找出当前entity下的mention个数
                        sent_id = int(entity_list[j][k]['sent_id']) # 找出当前这个mention 所在的sent_id 
                        ...
                        entity_id2pos[j] = min(entity_list[j][k]['global_pos'][0],entity_id2pos.get(j,999999))
                        
                        entity_id2type[j] = entity_list[j][k]['type']

                coref_pos = {} # 存储每个指示代词在文中的开始位置（以start word）
                for j in range(len(coreference)):
                    # 遍历每一个指示代词
                    sent_id = coreference[j]['sent_id']
                    coref_start = coreference[j]['pos'][0] + Ls[sent_id]  
                    coref_pos[j] = coref_start 
                ...
                # TODO: => 其实负样本的选择也是很重要的，所以这里也是一个课题可以作为改进
                na_triple = []
                for j in range(len(entity_list)):
                    for k in range(len(entity_list)):
                        # 确保最后一个样本是放到了负样本中 => 为了后期构图方便
                        if (j != k and (j, k) not in train_triple) or (j==len(entity_list)-1 and j==k+1):  # 如果不在正样本数据中，则作为负样本使用
                            na_triple.append((j, k))

                # generate document ids
                words = [] # 取出sentence 中的所有单词
                #TODO: 需要考虑这里直接拼接会不会丢失句间的这种关系？
                ...
                ner_id = np.zeros((self.document_max_length,), dtype=np.int32) # 默认的初始值(0)就代表这个不是NER
                
                # 每个位置对应的token 是否属于一个mention，如果是一个mention，那么就会得到写上对应的mention_id
                pos2mention_id = np.zeros((self.document_max_length,), dtype=np.int32)
                ...
                for idx, vertex in enumerate(entity_list, 1): # 对所有的entity遍历
                    for v in vertex: # vertex 代表的是这个entity的所有 mention 集合
                        ...
                        pos2mention_id[pos0:pos1] = mention_idx # 记录从pos:pos1 这个位置是一个mention（用的是mention_idx下标），对应第mention_idx个mention
                ...
                if entity2mention[idx] == []:                    
                    entity2mention[idx].append(mention_idx)
                    while pos2mention_id[replace_i] != 0:
                        replace_i += 1
                    pos2mention_id[replace_i] = mention_idx
                    pos2entityid[replace_i] = idx
                    ner_id[replace_i] = ner2id[vertex[0]['type']]
                    mention_idx += 1

                # 找出指示代词在分词后位置的下标
                pronoun_id = np.zeros((self.document_max_length,), dtype=np.int32)  
                for idx, pronouns in enumerate(coreference, 1):
                    sent_id, (start, end) = pronouns['sent_id'],pronouns['pos']  # 注意这里取的是绝对位置
                    pos0 = Ls[sent_id] + start # 得到 global 级的位置数据
                    # pos1 = Ls[sent_id] + end
                    pos0 = bert_starts[pos0] # 返回在tokenizer之后（原words在）位置pos0处的位置
                    pos1 = pos0 + 1
                ...
                # 添加 coreference mask position matrix 信息
                # 生成一个 (entity_num * coref_num)的矩阵信息
                entity_num = len(entity_list)
                coref_num = len(coreference)
                entity_coref_mask = [[0 for i in range(coref_num)] for j in range(entity_num)]
                                                
                
                # 根据 she/he => PER, it => LOC/ORG 规则添加限制信息
                # 根据实体位置关系和 coreference 的位置关系，判别两个是否要mask
                def _add_constraint(entity_coref_mask,entity_id2type,entity_id2pos):                    
                    for i in range(entity_num):
                        cur_entity_pos = entity_id2pos[i] # 获取得到当前实体的相对位置信息
                        cur_entity_type = entity_id2type[i] # 获得当前实体的类型
                        for j in range(coref_num):
                            cur_coref_pos = coref_pos[j]
                            cur_coref_name = coreference[j]['name']
                         
                            if cur_entity_pos > cur_coref_pos:# 根据实体位置关系和 coreference 的位置关系，判别两个是否要mask
                                entity_coref_mask[i][j] = 1 # 这个代词不可能指代这个entity

                            # {"BLANK": 0, "ORG": 1, "LOC": 2, "TIME": 3, "PER": 4, "MISC": 5, "NUM": 6}
                            # 根据指代类型来mask 
                            if (cur_coref_name == 'it' or cur_coref_name == 'its' ) and cur_entity_type not in ['LOC','ORG','MISC'] : 
                                entity_coref_mask[i][j] = 1                             

                            elif cur_coref_name in ['she','her','him','his','he'] and cur_entity_type in ['LOC','ORG','NUM','MISC','TIME']: 
                                entity_coref_mask[i][j] = 1
                    # 将数据做下padding
                    return entity_coref_mask

                entity_coref_mask = _add_constraint(entity_coref_mask,entity_id2type,entity_id2pos)
                self.data.append(
                    {
                    'title': title,
                    ...
                    'entity_coref_mask':entity_coref_mask,
...
class DGLREDataloader(DataLoader):
    ...
    def __init__(self, dataset, batch_size, shuffle=False, h_t_limit_per_batch=300, h_t_limit=1722, relation_num=97,max_length=512, negativa_alpha=0.0, dataset_type='train',pos_lsr = 0.9,neg_lsr = 0.9):
        ...
        self.dis2idx[256:] = 9
        self.dis_size = 20
        self.pos_lsr = pos_lsr # 正样本的lsr
        self.neg_lsr = neg_lsr # 负样本的lsr 
    ...
    def __iter__(self):
        ...
        for idx, minibatch in enumerate(self.batches):
            ...
            pronoun_id = torch.zeros(self.batch_size, self.max_length,dtype=torch.long).cpu()
            # entity_coref_mask = torch.zeros(self.batch_size, 50, 20 ,dtype=torch.long).cpu() # 50表示最多实体数，20表示最大指示代词数
            ...
            # 每篇 doc 形成的一个矩阵（entity 和 mention的对应关系）叫做entity2mention。将batch下的每篇doc得到的矩阵放到同一个list中
            entity2mention_table = [] 
            entity_coref_mask = [] 
            
            batch_entity_id = []
            overlaps = []
            entity_name = [] # 当前这个batch中所有entity的名字
            # 对这个batch 中的数据进行处理
            for i, example in enumerate(minibatch):
                ...
                for j in range(len(entities)):
                    cur_entity_name.append(entities[j][0]['name'])
                entity_name.append(cur_entity_name)
                # 得到当前doc的 指示代词的信息
                cur_pronoun_id = example['pronoun_id']
                cur_entity_coref_mask = example['entity_coref_mask']
                entity_coref_mask.append(get_cuda(torch.tensor(cur_entity_coref_mask)))
                ...
                entity2mention_table.append(entity2mention_t) # GAIN源码
                # entity2mention_table.append(entity2mention_t.tolist()) # 因为我不想让其用tensor，所以这里就用list保存
                ...
                pronoun_id[i,:].copy_(torch.from_numpy(cur_pronoun_id))
                ...
                if self.dataset_type == 'train':
                    ...
                    for j, (h_idx, t_idx) in enumerate(train_triple):
                        ...
                        global_h_entity_idx = self.entity2id.get(h_entity_name,-1)
                        global_t_entity_idx = self.entity2id.get(t_entity_name,-1)
                        ...
                        for r in label: # 可能一对实体有多个 relation，所以这里有个for循环
                            if label_id2name[r] in more_label:
                                relation_multi_label[i, j, r] = self.pos_lsr
                            else:
                    ...
                    # new_na_triple = na_triple[0:-1] # 最后一位不shuffle
                    # 下面这个if是 让lower_bound 变小，但是没有必要，我们把全部的负样本都算进去
                    if self.negativa_alpha > 0.0: # self.negativa_alpha = 4.0
                        random.shuffle(na_triple) # 随机 shuffle
                        lower_bound = int(max(20, len(train_triple) * self.negativa_alpha)) # 确定负样本个数的下界，GAIN 设置的是lower_bound是20
                    # na_triple[-1:] 是为了保留最大节点的id，为了方便后面创建图使用
                    # 重复上面这个逻辑，只不过这里是针对负样本的。注意这里 len(train_triple) 的值会被初始化为j
                    # na_triple[:lower_bound] 即使这个lower_bound 超过了 na_triple 自身的长度，也依然不会报错
                    for j, (h_idx, t_idx) in enumerate(na_triple[:lower_bound], len(train_triple)):
                        ...
                        relation_multi_label[i, j, 0] = self.neg_lsr # lsr 策略
            ...
            yield {'context_idxs': get_cuda(context_word_ids[:cur_bsz, :batch_max_length].contiguous()),
                   ...
                   'pronoun_id':get_cuda(pronoun_id),
                   'entity_coref_mask':entity_coref_mask,
                   'entity_name':entity_name # 输出entity的name信息
                   }