...
if true; then
  model_name=BERT_BASE_Rdrop_T_cs_type_loss
  lr=1e-3
  lr_bert=2e-5  
  lr_tran=1e-4
  batch_size=8
  ...
  nohup python -u train_bert_rdrop_t_cs_type_loss.py \
    --notes "结合coslr重新跑" \
    --train_set ../data/train.json \
    --train_set_save ../data/prepro_data/train_BERT_T_uncased_type_loss.pkl \
    --dev_set ../data/dev.json \
    --dev_set_save ../data/prepro_data/dev_BERT_T_uncased_type_loss.pkl \
