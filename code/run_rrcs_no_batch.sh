...
if true; then
  ...
  date=`date +%Y%m%d`
  time=`date +%H%M%S`
  fig_result_dir=logs/${date}/${model_name}_neg_alpha_${negativa_alpha}_epoch_${epoch}_lr_${lr_rgcn}_${lr_bert}_${lr}_gthreshold_${graph_threshold}_dropout_${dropout}_${time}
  ...
  nohup python -u train_rrcs_no_batch.py \
    ...
    --fig_file_name RRCS_BERT_BASE_no_batch \
    --use_model bert \
