...
class BERTDGLREDataset(IterableDataset):
    def __init__(self, src_file, save_file, word2id, ner2id, rel2id,
        ...
        if os.path.exists(save_file):
            ...
            for i, doc in enumerate(ori_data):
                ...
                L = 0
                word2type = [] # 得到sentence中每个单词的类型，这个数据用于给后面的 is_capital()使用
                # step1. 遍历每个的 sentence 的长度，累计得到当前的总长度L， 并将其放到 Ls中。
                for x in sentences: 
                    L += len(x)
                    Ls.append(L)
                    word2type.append([0]*len(x))
                ...
                for j in range(len(entity_list)): 
                    for k in range(len(entity_list[j])): # 找出当前entity下的mention个数
                        ...
                        type = entity_list[j][k]['type']
                        if type == 'MISC':# 说明是MISC类型
                            sent_id = entity_list[j][k]['sent_id']
                            word2type[sent_id][mention_start:mention_end] = [3]*(mention_end-mention_start)
                        elif type == 'LOC':
                            sent_id = entity_list[j][k]['sent_id']
                            word2type[sent_id][mention_start:mention_end] = [1]*(mention_end-mention_start)
                # step3. 遍历labels
                # generate positive examples
                train_triple = []
                ...
                for i in range(len(sentences)):
                    ...
                    word2type_flat.extend(word2type[i])
                assert len(words) == len(word2type_flat)
                # for i in zip(words,word2type_flat):
                #     print(i)
                ...
                # capital_mask 用于标记该位置是否是大写字符
                a,capital_mask = self.is_capital(words,word2type_flat)
                assert a == bert_subwords
    ...
    # 检测该字符是否是大写的
    # 传入的是原始字符 words
    def is_capital(self,words,word2type_flat):
        subwords = ['[CLS]']
        # print(subwords)
        # 得到subwords之后，再开始判断各个word的大小写情况
        upper_mask = [0] # 用于标记一串token是否是大小写的信息 
        pre_word = None
        for i in range(len(words)): # 对每个单词进行处理
            word = words[i] # 得到第i个单词        
            tmp = self.tokenizer.tokenize(word)
                        
            # 如果是特殊类型，则加上偏置，否则不加 【这里的特殊类型可能是MISC，也可能是LOC】            
            subwords.extend(tmp)
            if word[0].isupper() and pre_word != "." and word2type_flat[i]:
                upper_mask.extend([1]*len(tmp))
            else:
                upper_mask.extend([0]*len(tmp))
            
            # 还要判断是否超出边界，如果超出界限，则直接取到510
            if len(subwords)>=510:
                subwords = subwords[:510]
                upper_mask = upper_mask[:510]
                break

            pre_word = word # 得到上一次的字符
        subwords.append("[SEP]")
        upper_mask.append(0)
        return (subwords,upper_mask)
...
class DGLREDataloader(DataLoader):
    ...
    def __iter__(self):
        ...
        for idx, minibatch in enumerate(self.batches):
            ...
            capital_mask = torch.zeros(self.batch_size, self.max_length,dtype=torch.float).cpu()  # 后面参与计算，设置成float类型
            ...
            for i, example in enumerate(minibatch):
                title, entities, labels, na_triple, word_id, pos_id, ner_id, mention_id, entity2mention, entity_id, cur_capital_mask = \
                    ...
                    example['word_id'], example['pos_id'], example['ner_id'], example['mention_id'], example[
                        'entity2mention'],example['entity_id'],example['capital_mask']
                overlaps.append(example['overlap'])
                # capital_mask = torch.tensor(capital_mask)
                capital_mask[i,:len(cur_capital_mask)] = torch.tensor(cur_capital_mask)
            ...
            yield {'context_idxs': get_cuda(context_word_ids[:cur_bsz, :batch_max_length].contiguous()),
                   ...
                   'capital_mask':get_cuda(capital_mask[:cur_bsz, :batch_max_length].contiguous())
                   }