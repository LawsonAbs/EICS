...
03.加入 type*type*relation 作为 loss
'''
...
from config import *
from data_RGCN_type_loss_global import DGLREDataset, DGLREDataloader, BERTDGLREDataset  # 这是从data.py 中引入几个类
from models.BERT_rdrop_t_cs import  BERT_T
from evaluate_bert_rdrop_t_cs import eval
from utils import Accuracy, get_cuda, logging, print_params,get_all_entity,get_label2id,get_labelid2name,get_type_relation_lable_from_train
...
type_relation_label = torch.tensor(get_type_relation_lable_from_train()).cuda()
type_relation_label = type_relation_label/ torch.sum(type_relation_label,dim=-1,keepdim=True)
...
def get_info_from_logits(predictions,h_t_pairs_entity_type_id,relation_mask):
    # type_type_relation_in_pred =  torch.zeros(6*6, 97).cuda(device)    
    predictions = torch.sigmoid(predictions.view(-1,97))  # [total_pair_num,97]
    # TODO: 还需要根据 predictions mask 选择处理
    # idxs size [batch_size,max_entity_pair_num]，表示的含义就是：每对实体得到映射的下标值
    idxs = (h_t_pairs_entity_type_id[:,:,0] - 1)*6 + (h_t_pairs_entity_type_id[:,:,1] - 1)
    idxs = idxs.view(-1) # [total_pair_num]

    idx_2 = idxs.unsqueeze(-1).expand(-1,36)
    mask = torch.arange(36).unsqueeze(0).expand(predictions.size(0),-1).cuda()
    select_matrix = ((mask==idx_2)+0.0).T # size [36,predictions.size(0)]
    # target[i][j]: 第i种实体类型的组合在第j类relation下的概率
    target = select_matrix @ predictions # size [36,97]
...
def train(opt):
    ...
    for epoch in range(start_epoch, opt.epoch + 1):
        ...
        for ii, d in enumerate(train_loader): # 送入模型的是一个batch 的数据
            ...
            # 因为我没有过分优化负样本，所以负样本的可能性远没有实际的频率那么高，所以这里去除掉了NA类，仅对正样本的分布做一个学习            
            type_type_relation_logits = get_info_from_logits(predictions_bert_2,h_t_pairs_entity_type_id,relation_mask)
            loss_type = torch.sum((type_type_relation_logits[:,1::]-type_relation_label[:,1::])**2) # MSE
            
            loss = (loss_bert_1 + loss_bert_2)/2  + kl + opt.loss_type_weight*loss_type
            ...
            if global_step % log_step == 0:
                ...
                logging(
                    '| epoch {:2d} | step {:4d} |  ms/b {:5.2f} | train loss {:5.3f} | type loss {:5.4f} | NA acc: {:4.2f} | PA acc: {:4.2f}  | tot acc: {:4.2f} '.format(
                        epoch, global_step, elapsed * 1000 / log_step, cur_loss * 1000, cur_loss_type ,acc_NA.get(), acc_PA.get(),
                        acc_total.get()))
...
if __name__ == '__main__':
    ...
    wandb.init(project="train_bert_rdrop_t_cs_type_loss", entity="lawsonabs")
