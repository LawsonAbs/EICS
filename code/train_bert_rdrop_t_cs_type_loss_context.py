...
from config import *
from data_RGCN_type_loss_context import DGLREDataset, DGLREDataloader, BERTDGLREDataset  # 这是从data.py 中引入几个类
from models.BERT_rdrop_t_cs_type_loss_context import  BERT_T
    ...
    for epoch in range(start_epoch, opt.epoch + 1):
        ...
        for ii, d in enumerate(train_loader): # 送入模型的是一个batch 的数据
            ...
            if global_step % log_step == 0:
                ...
                logging(
                    '| epoch {:2d} | step {:4d} |  ms/b {:5.2f} | train loss {:5.3f} | loss type {:5.4f} | NA acc: {:4.2f} | PA acc: {:4.2f}  | tot acc: {:4.2f} '.format(
        ...
        if epoch % opt.dev_period == 0 and acc_PA.get() >= 0.5:
            ...
            # 先由bert得到初步结果，           
            ign_f1, ign_auc, pr_x, pr_y, dev_result = eval_cons(bert_T,dev_loader, model_name, id2rel=id2rel)
...
if __name__ == '__main__':
    ...
    wandb.init(project="train_bert_rdrop_t_cs_type_loss_context", entity="lawsonabs")
