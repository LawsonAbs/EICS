'''
1.在bert后先添加transformer
2.使用bert中的低层embedding，主要是获取预训练模型的常识知识。使用新的transformer 层来解决这个问题。
...
from config import *
# from data_RGCN_capital import DGLREDataset, DGLREDataloader, BERTDGLREDataset  # 这是从data.py 中引入几个类
from data_RGCN import DGLREDataset, DGLREDataloader, BERTDGLREDataset  # 这是从data.py 中引入几个类
# from data_T_mask_2 import DGLREDataset, DGLREDataloader, BERTDGLREDataset  
# from models.BERT_rdrop_t_cs_Gaussian import  BERT_T
from models.BERT_rdrop_t_cs_word_embedding_combine import  BERT_T
...
def train(opt):
    ...
    if opt.use_model == 'bert':
        ...
        transformer_param_ids = list(map(id, bert_T.bert_encoder.parameters())) #+ list(map(id, bert_T.bert_encoder_2.parameters())) # 获取bert模型的参数
        # word_embeddings = list(map(id,bert_T.word_embeddings.parameters()))
        # gate_params = list(map(id,bert_T.gate_0))
        weights_ids = [id(bert_T.weights)]
        # TODO: 验证一下逻辑是否正确
        # 对 model.parameters() 参数进行一个过滤操作
        # 这个操作最后得到的结果不应该是空的吗？ => 不是，会对 线性层等参数进行过滤
        # 返回对象是一个filter
        linar_params = filter(lambda p: p.requires_grad and id(p) not in bert_param_ids+transformer_param_ids+weights_ids, bert_T.parameters())

        optimizer = optim.AdamW([
            {'params': bert_T.bert.parameters(), 'lr': opt.lr_bert},
            {'params': bert_T.bert_encoder.parameters(), 'lr': opt.lr_tran},
            {'params': bert_T.weights, 'lr': opt.lr_weight},
    ...
    if opt.coslr:
        scheduler = optim.lr_scheduler.CosineAnnealingLR(optimizer, T_max= opt.epoch  + 1)
    ...
    for epoch in range(start_epoch, opt.epoch + 1):
        ...
        for ii, d in enumerate(train_loader): # 送入模型的是一个batch 的数据
            ...
            predictions_bert_1,layer_attention_score_1 = bert_T(words=d['context_idxs'],
            ...
            # 第二次使用 bert 
            predictions_bert_2,layer_attention_score_2 = bert_T(words=d['context_idxs'],
                                src_lengths=d['context_word_length'],
                                mask=d['context_word_mask'],
                                entity_type=d['context_ner'],
                                entity_id=d['context_pos'],
            ...
            # loss_cs_1 = cal_focal_loss(predictions_cs_1,relation_multi_label,opt.gamma,relation_mask)
            # loss_cs_2 = cal_focal_loss(predictions_cs_2,relation_multi_label,opt.gamma,relation_mask)                


            # 再计算两者的 KL散度
            kl_1=(    F.kl_div(F.log_softmax(predictions_bert_1, dim=-1),F.softmax(predictions_bert_2,dim=-1)) +
                        F.kl_div(F.log_softmax(predictions_bert_2,dim=-1),F.softmax(predictions_bert_1,dim=-1))
                    )/2
            
            # kl_2=(    F.kl_div(F.log_softmax(predictions_cs_1, dim=-1),F.softmax(predictions_cs_2,dim=-1)) +
            #             F.kl_div(F.log_softmax(predictions_cs_2,dim=-1),F.softmax(predictions_cs_1,dim=-1))
            #         )/2

            loss = (loss_bert_1 + loss_bert_2)/2  + kl_1
        ...
        if acc_PA.get() >= 0.5 and acc_PA.get() < 0.8:
            opt.dev_period = 4
        elif acc_PA.get() >= 0.8 and acc_PA.get() < 0.9: 
            opt.dev_period = 2
        else:
            opt.dev_period = 1
...
if __name__ == '__main__':
    ...
    wandb.init(project="train_bert_rdrop_t_cs_final", entity="lawsonabs")
