            ...
            for i, doc in enumerate(ori_data):
                ...
                for idx, vertex in enumerate(entity_list, 1): # 对所有的entity遍历
                    for v in vertex: # vertex 代表的是这个entity的所有 mention 集合
                        ...
                        # 下面这两种if会存在吗？ 会存在
                        # 第一个if在 title = 'William Busac' 这个doc中就会出现，但是标注的这两个具有不同的实体
                        # 说明指向的是同一个mention，这种情况怎么处理？
                        if (pos0, pos1) in already_exist.keys():
                            # 判断当前的第 idx 个实体 是否有对应的mention_idx，如果没有，则需要手动补一个进去
                            if(len(entity2mention[idx]) == 0):
                                entity2mention[idx].append(already_exist[(pos0, pos1)])  
                            else: # 如果有，则下一个
                                continue
                        
                        # 第二种会在超出512 的长度的时候出现，该怎么挽救？
                        if pos0 >= len(pos2entityid):
                            if idx<len(entity_list): 
                                # TODO: 暂且用0替代（这里的0后面即视作是CLS向量），最后一个留给后面的while循环处理
                                entity2mention[idx].append(0)
                            continue
                        ...
                        entity2mention[idx].append(mention_idx)  
                        already_exist[(pos0, pos1)] = mention_idx
                        mention_idx += 1                    
                        
                replace_i = 0
                # 下面这个if的作用是 => 因为上面存在一个continue，所以可能导致某些的 entity2mention[idx] 是没有值的
                idx = len(entity_list) 
...
class DGLREDataloader(DataLoader):
    ...
    def __init__(self, dataset, batch_size, shuffle=False, h_t_limit_per_batch=300, h_t_limit=1722, relation_num=97,max_length=512, negativa_alpha=0.0, dataset_type='train',lsr = 0.9):
    ...
    def __iter__(self):
        ...
        for idx, minibatch in enumerate(self.batches):
            ...
            entity2mention_batch = [] # 当前这个batch 中每篇doc 的entity2mention 信息
            batch_entity_id = []
            overlaps = []
            # 对这个batch 中的数据进行处理
            for i, example in enumerate(minibatch):
                ...
                entity2mention_batch.append(entity2mention) 
                    ...
                    for h_idx in range(L): # 因为要判断所有的entity，所以用的是双重for循环
                        for t_idx in range(L):
                            if h_idx != t_idx: # 这里判断是否赋值 ht_pairs 的条件只是 h_idx 和 t_idx 的值的比较
                                ...
                                if(h_idx,t_idx) not in idx2label.keys(): # 负样本
                                    relation_mask[i, j] = 1
                                    relation_multi_label[i, j, 0] = 0.9 # lsr 策略 => 避免负样本的过度学习
                                    ...
                                    for r in label: # 可能一对实体有多个 relation，所以这里有个for循环
                                        if label_id2name[r] in more_label:
                                            relation_multi_label[i, j, r] = self.lsr
                                        else:
            ...
            yield {'context_idxs': get_cuda(context_word_ids[:cur_bsz, :batch_max_length].contiguous()),
                   ...
                   'entity2mention_batch':entity2mention_batch,
                   'overlaps': overlaps,
                   }