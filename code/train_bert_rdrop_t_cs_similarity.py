...
2. 使用bert中的低层embedding，主要是获取预训练模型的常识知识。
3. 使用 sim_loss 控制relation 表示的距离
'''
...
from models.BERT_rdrop_t_cs_similarity import  BERT_T
...
def train(opt):
    ...
    for epoch in range(start_epoch, opt.epoch + 1):
        ...
        for ii, d in enumerate(train_loader): # 送入模型的是一个batch 的数据
            ...
            loss = (loss_bert_1 + loss_bert_2)/2  + kl + sim_loss_1
