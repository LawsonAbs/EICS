    ...
    def __init__(self, src_file, save_file, word2id, ner2id, rel2id,
        ...
        if os.path.exists(save_file):
            ...
            for i, doc in enumerate(ori_data):
                ...
                entity2pronoun = defaultdict(list) # 记住每个entity 对应 pronoun 的id
                ...
                for idx, vertex in enumerate(entity_list, 1): # 对所有的entity遍历
                    for v in vertex: # vertex 代表的是这个entity的所有 mention 集合
                        ...
                        # 如果当前是指示代词的信息，就不要再写成mention，而是写成pronoun中
                        if ner_type == 'pronoun':
                            pronoun_id[pos0:pos1] = pronoun_idx
                            entity2pronoun[idx].append(pronoun_idx)  # 实体到指示代词的映射
                            pronoun_idx+=1
                        else:
...
class DGLREDataloader(DataLoader):
    ...
    def __iter__(self):
        ...
        for idx, minibatch in enumerate(self.batches):
            ...
            pronoun_ids = torch.zeros(self.batch_size, self.max_length,dtype=torch.long).cpu()
            ...
            entity2mention_table = [] # 每篇 doc 形成的一个矩阵（entity 和 mention的对应关系）叫做entity2mention。将batch下的每篇doc得到的矩阵放到同一个list中
            entity2pronoun_table = [] # entity 到 pronoun 的映射
            batch_entity_id = []
            overlaps = []
            # 对这个batch 中的数据进行处理
            for i, example in enumerate(minibatch):
                ...
                entity2pronoun , pronoun_id = example['entity2pronoun'],example['pronoun_id']
                overlaps.append(example['overlap'])
                # entity2mention_t 是个矩阵的形式，为啥要多加1？ => 因为第一个是留给cls,所以+1                
                entity2mention_t = get_cuda(torch.zeros((pos_id.max() + 1, mention_id.max() + 1)))
                entity2pronoun_t = get_cuda(torch.zeros((pos_id.max() + 1, pronoun_id.max() + 1)))
                ...
                for e,ms in entity2pronoun.items():
                    for m in ms:
                        entity2pronoun_t[e, m] = 1


                entity2mention_table.append(entity2mention_t)
                entity2pronoun_table.append(entity2pronoun_t)
                ...
                pronoun_ids[i, :word_num].copy_(torch.from_numpy(pronoun_id))
            ...
            yield {'context_idxs': get_cuda(context_word_ids[:cur_bsz, :batch_max_length].contiguous()),
                   ...
                   'entity2pronoun_table': entity2pronoun_table,
                   'pronoun_id':get_cuda(pronoun_ids[:cur_bsz, :batch_max_length].contiguous()),
                   'overlaps': overlaps,
