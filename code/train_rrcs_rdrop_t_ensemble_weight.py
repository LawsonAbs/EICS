'''
在bert后先添加transformer，再过RGCN
(1)使用transformer 的预测值 和 RGCN的预测值来做一个合并，相当于一个 ensemble
'''
...
from models.RRCS_rdrop_t_ensemble_weight import  BERT_T,RRCS ,create_heterograph
from evaluate_rrcs_rdrop_t_ensemble_weight import eval
from utils import Accuracy, get_cuda, logging, print_params,get_all_entity,get_label2id,get_labelid2name,get_label_map
...
label_info = get_label_map('../data/rel_info.json')
...
def train(opt):
    ...
    if opt.use_model == 'bert':
        ...
        rgcn_params_ids =  list(map(id, rrcs.RGCN.parameters())) # 获取RGCN 的参数
        # TODO: 验证一下逻辑是否正确
        ...
        rgcn_base_params = filter(lambda p: p.requires_grad and id(p) not in rgcn_params_ids, rrcs.parameters())

        optimizer = optim.AdamW([
            {'params': bert_T.bert.parameters(), 'lr': opt.lr_bert},
            {'params': bert_T.bert_encoder.parameters(), 'lr': opt.lr_bert},
            {'params': rrcs.RGCN.parameters(), 'lr': opt.lr_rgcn},
            {'params': linar_params, 'lr': opt.lr}, 
            {'params': rgcn_base_params, 'lr': opt.lr},
    ...
    for epoch in range(start_epoch, opt.epoch + 1):
        ...
        for ii, d in enumerate(train_loader): # 送入模型的是一个batch 的数据
            ...
            cur_bsz = relation_multi_label.size(0) # 获取当前batch 大小
            # 验证参数是否更新
            ...
            predictions_bert_1,batch_entity_feature,batch_entity_feature_transformer = bert_T(words=d['context_idxs'],
            ...
            # 第二次使用 bert 
            predictions_bert_2,_,_ = bert_T(words=d['context_idxs'],
                                src_lengths=d['context_word_length'],
                                mask=d['context_word_mask'],
                                entity_type=d['context_ner'],
                                entity_id=d['context_pos'],
            ...
            # 根据得到的关系建图
            h_t_pairs = d['h_t_pairs']
            h_t_pairs = h_t_pairs + (h_t_pairs == 0).long() - 1  # 处理一下 h_t_pairs 
            entity_graphs = create_heterograph(h_t_pairs,
                                        predictions_bert_1,
                                        opt.graph_threshold,
                                        relid2name,
                                        h_t_type=None,
                                        batch_entity_num = torch.max(d['context_pos'],dim=1)[0].tolist()
                                        )
            graph_etype_bert = [entity_graphs[i].etypes for i in range(len(entity_graphs))]            
            graph_etype_info_bert = []
            graph_enums_bert = [entity_graphs[i].num_edges() for i in range(len(entity_graphs))]
            for i in range(cur_bsz):
                cur_graph_etype_info = []
                cur_graph_etype = graph_etype_bert[i]
                for etype in cur_graph_etype:
                    etype_info = label_info[etype]
                    cur_graph_etype_info.append((etype,etype_info))
                graph_etype_info_bert.append(cur_graph_etype_info)
            print(f"前：{sum(graph_enums_bert)},",graph_etype_info_bert)

            # 对上面两个的计算结果进行一个处理
            predictions_rgcn = rrcs(words=d['context_idxs'],
                                src_lengths=d['context_word_length'],
                                mask=d['context_word_mask'],
                                entity_type=d['context_ner'],
                                entity_id=d['context_pos'],                                
                                ...
                                batch_entity_feature = batch_entity_feature,
                                batch_entity_feature_transformer = batch_entity_feature_transformer                                
                                )
            # 验证过完RGCN 之后创建的图，对比二者的区别
            entity_graphs_2 = create_heterograph(h_t_pairs,
                                        predictions_rgcn,
                                        opt.graph_threshold,
                                        relid2name,
                                        h_t_type=None,
            ...
            graph_etype_rgcn = [entity_graphs_2[i].etypes for i in range(len(entity_graphs_2))]
            graph_enums_rgcn = [entity_graphs_2[i].num_edges() for i in range(len(entity_graphs_2))]
            graph_etype_info_rgcn = []
            for i in range(cur_bsz):
                cur_graph_etype_info = []
                cur_graph_etype = graph_etype_rgcn[i]
                for etype in cur_graph_etype:
                    etype_info = label_info[etype]
                    cur_graph_etype_info.append((etype,etype_info))
                graph_etype_info_rgcn.append(cur_graph_etype_info)
            print(f"后：{sum(graph_enums_rgcn)},",graph_etype_info_rgcn,end="\n\n")
            # 使用 rdrop 
            ...
            loss_2 = torch.sum(BCE(predictions_rgcn, relation_multi_label) * relation_mask.unsqueeze(2)) / (
                    opt.relation_num * torch.sum(relation_mask)) 
            loss = (loss_bert_1 + loss_bert_2 + loss_2)/2  + kl
            ...
            # 对BERT 和 RGCN 的预测结果 ensemble 
            # 下面这个代码是有问题的。如果不乘系数直接加会导致有关系的关系分数更大，没有关系的关系分数很小
        ...
        if epoch % opt.dev_period == 0 and acc_PA.get() >= 0.9:
            ...
            # 先由bert得到初步结果，再由rrcs得到最终结果            
            ign_f1, ign_auc, pr_x, pr_y, dev_result = eval(bert_T,rrcs, dev_loader,opt, model_name, id2rel=id2rel)
            ...
            if ign_f1 > best_ign_f1:
                ...
                torch.save({
                    ...
                    'checkpoint_rrcs': rrcs.state_dict(),
                    'lr': lr,
                    'best_ign_f1': ign_f1,
...
if __name__ == '__main__':
    ...
    wandb.init(project="train_rrcs_rdrop_t_ensemble_weight", entity="lawsonabs")
