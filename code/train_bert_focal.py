'''
BASELINE: 使用bert作为baseline，同时在其上使用focal loss
'''
import wandb
import time
...
def train(opt):
    ...
    for epoch in range(start_epoch, opt.epoch + 1):
        ...
        for ii, d in enumerate(train_loader): # 送入模型的是一个batch 的数据
            ...
            loss = cal_focal_loss(predictions_bert,relation_multi_label,opt.gamma,relation_mask)
            # loss 表示的是第一个阶段（使用 BERT） 计算得到的损失                 
...
if __name__ == '__main__':
    ...
    wandb.init(project="train_bert_focal", entity="lawsonabs")
