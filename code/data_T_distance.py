...
class DGLREDataloader(DataLoader):
    ...
    def __init__(self, dataset, batch_size, shuffle=False, h_t_limit_per_batch=300, h_t_limit=1722, relation_num=97,max_length=512, negativa_alpha=0.0, dataset_type='train',window = 100):
    ...
    def __iter__(self):
        ...
        for idx, minibatch in enumerate(self.batches):
            ...
            x = torch.arange(self.max_length).unsqueeze(1).expand(-1,self.max_length) # 行坐标
            y = torch.arange(self.max_length).unsqueeze(0).expand(self.max_length,-1) # 纵坐标

            # 根据window(w) 修改矩阵的大小            
            # 生成 length mask，过滤掉不符合要求的
            val = torch.abs(x-y)
    
            # length_mask = (val<=self.window)+0
            tmp = (val>self.window)+0
            INF = torch.tensor([-10000]).unsqueeze(1).expand(tmp.size(0),tmp.size(1))
            length_mask = INF*tmp 
            ...
            yield {'context_idxs': get_cuda(context_word_ids[:cur_bsz, :batch_max_length].contiguous()),
                   ...
                   'length_mask':get_cuda(length_mask[0:batch_max_length,0:batch_max_length])
                   }