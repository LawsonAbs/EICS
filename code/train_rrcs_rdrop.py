...
from models.RRCS_rdrop import  BERT,RRCS ,create_heterograph
from evaluate_rrcs_rdrop import eval
...
def train(opt):
    ...
    bert = get_cuda(bert) # 将模型放到GPU中
    rrcs = rrcs.cuda() # 将模型放到GPU中
    if opt.use_model == 'bert':
        bert_param_ids = list(map(id, bert.bert.parameters())) # 获取bert模型的参数 
        ...
        # 返回对象是一个filter
        bert_base_params = filter(lambda p: p.requires_grad and id(p) not in bert_param_ids, bert.parameters())

        rgcn_base_params = filter(lambda p: p.requires_grad and id(p) not in rgcn_params_ids, rrcs.parameters())

        optimizer = optim.AdamW([
            {'params': bert.bert.parameters(), 'lr': opt.lr_bert}, 
            {'params': rrcs.RGCN.parameters(), 'lr': opt.lr_rgcn},
            {'params': bert_base_params, 'lr': opt.lr}, 
    ...
    for epoch in range(start_epoch, opt.epoch + 1):
        ...
        for ii, d in enumerate(train_loader): # 送入模型的是一个batch 的数据
            ...
            predictions_bert_1,batch_feature_bert_1 = bert(words=d['context_idxs'],
            ...
            # 第二次使用 bert 
            predictions_bert_2,_ = bert(words=d['context_idxs'],
                                src_lengths=d['context_word_length'],
                                mask=d['context_word_mask'],
                                entity_type=d['context_ner'],
                                entity_id=d['context_pos'],
            ...
            predictions_rgcn = rrcs(words=d['context_idxs'],
                                ...
                                batch_feature_bert = batch_feature_bert_1
                                )
            ...
            if opt.clip != -1:
                nn.utils.clip_grad_value_(bert.parameters(), opt.clip)
            optimizer.step()
            # if opt.coslr:  # 原本是在用的
        ...
        if epoch % opt.dev_period == 0 and acc_PA.get() >= 0.9:
            ...
            ign_f1, ign_auc, pr_x, pr_y, dev_result = eval(bert,rrcs, dev_loader,opt, model_name, id2rel=id2rel)
            bert.train() # 再设置成train()
            rrcs.train()
            ...
            if ign_f1 > best_ign_f1:
                ...
                torch.save({
                    'epoch': epoch,
                    'checkpoint_bert': bert.state_dict(),
...
if __name__ == '__main__':
    ...
    wandb.init(project="train_rrcs_rdrop", entity="lawsonabs")
