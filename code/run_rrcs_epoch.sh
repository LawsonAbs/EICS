...
if true; then
  ...
  date=`date +%Y%m%d`
  time=`date +%H%M%S`
  fig_result_dir=logs/${date}/${model_name}_neg_alpha_${negativa_alpha}_epoch_${epoch}_lr_${lr_rgcn}_${lr_bert}_${lr}_gthreshold_${graph_threshold}_${time}_513
  ...
  nohup python -u train_rrcs_epoch.py \
    --train_set ../data/train.json \
    --train_set_save ../data/prepro_data/train_BERT_RRCS_513.pkl \
    --dev_set ../data/dev.json \
    --dev_set_save ../data/prepro_data/dev_BERT_RRCS_44.pkl \
