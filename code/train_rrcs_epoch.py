...
from evaluate_rrcs_epoch import eval
...
def train(opt):
    ...
    for epoch in range(start_epoch, opt.epoch + 1):
        ...
        for ii, d in enumerate(train_loader): # 送入模型的是一个batch 的数据
            ...
            if epoch >= opt.epoch_threshold:
                # 如果达到一定的效果之后，就固定住 bert 的参数，不再训练
                for p in model.bert.parameters():
                    p.requires_grad = False
                

            # 除以(opt.relation_num * torch.sum(relation_mask)) 是因为想得到平均每个样本的损失
            loss = torch.sum(BCE(predictions, relation_multi_label) * relation_mask.unsqueeze(2)) / (opt.relation_num * torch.sum(relation_mask)) 
        ...
        if epoch % opt.dev_period == 0 and acc_PA.get() >= 0.8:
            ...
            model.eval() # 先设置成eval()
            ign_f1, ign_auc, pr_x, pr_y, dev_result = eval(epoch,opt,model, dev_loader, model_name, id2rel=id2rel)
...
if __name__ == '__main__':
    ...
    wandb.init(project="train_rrcs_epoch", entity="lawsonabs")
