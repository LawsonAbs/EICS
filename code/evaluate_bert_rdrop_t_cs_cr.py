...
def eval(bert,dataloader,opt, modelname, id2rel, input_theta=-1, output=False, is_test=False, test_prefix='dev',
    ...
    for cur_i, d in tqdm(enumerate(dataloader),total=len(dataloader)):    
        with torch.no_grad():
            ...
            batch_entities = d['entity_name'] # 当前这个batch 下所有entity的记录
            predictions_bert,batch_cr_scores = bert.forward(words=d['context_idxs'],
            ...
            print("\n")
            for i in zip(batch_cr_scores,batch_entities):
                for score,name in zip(*i):
...
if __name__ == '__main__':
    ...
    f1, auc, pr_x, pr_y ,dev_theta = eval(bert_t,    
                                        ...
                                        output=False,
                                        test_prefix='test_no_focal_loss',
                                        is_test=False,
                                        ours=False # TODO: ??
                                        )
    print('eval finished')