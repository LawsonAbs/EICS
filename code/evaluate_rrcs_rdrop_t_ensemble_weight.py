...
def eval(bert,rrcs, dataloader,opt, modelname, id2rel, input_theta=-1, output=False, is_test=False, test_prefix='dev',
    ...
    for cur_i, d in tqdm(enumerate(dataloader),total=len(dataloader)):    
        with torch.no_grad():
            ...
            predictions_bert,batch_entity_feature,batch_entity_feature_transformer = bert.forward(words=d['context_idxs'],
            ...
            predict_re = torch.sigmoid(predictions_rgcn)
...
if __name__ == '__main__':
    ...
    if pretrain_model != '':
        ...
        rrcs.load_state_dict(chkpt['checkpoint_rrcs'])
