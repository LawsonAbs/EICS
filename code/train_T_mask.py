'''
1.在 Dataloader 中动态使用 mask 机制，这样保证每个doc都不会被死死封锁。
2.不再使用[MASK]替换，而是使用与实体具有相同类型的实体进行替换。替换的原则可以参考 data_T_mask.py 源码部分
'''
import wandb
import time
...
from config import *
from data_T_mask import DGLREDataset, DGLREDataloader, BERTDGLREDataset  # 这是从data.py 中引入几个类
from models.BERT_T import BERT_T
from evaluate_T_constraint_mask import eval_cons
...
def train(opt):
    ...
    if opt.use_model == 'bert':
        ...
        train_loader = DGLREDataloader(train_set, batch_size=opt.batch_size, shuffle=True,
                                       negativa_alpha=opt.negativa_alpha,mask_prob=opt.mask_prob)
    ...
    for epoch in range(start_epoch, opt.epoch + 1):
        ...
        if acc_PA.get() >= 0.5 and acc_PA.get() < 0.9:
            opt.dev_period = 5
        elif acc_PA.get() >= 0.9 :
            opt.dev_period = 1
...
if __name__ == '__main__':
    ...
    wandb.init(project="train_bert_T_mask", entity="lawsonabs",config=opt)
