...
if true; then
  model_name=RRCS_BERT_BASE_Rdrop_T_ensemble_weight
  lr=1e-3
  lr_bert=2e-5
  lr_rgcn=1e-3
  batch_size=5
  ...
  base_num=97
  gcn_layers=2
  fig_result_dir=logs/${date}/${model_name}_neg_alpha_${negativa_alpha}_epoch_${epoch}_lr_${lr_rgcn}_${lr_bert}_${lr}_gthreshold_${graph_threshold}_dropout_${dropout}_bert_dropout_${bert_dropout}_${time}_lsr_${lsr}_interactLayres_${interaction_layers}_basenum_${base_num}_ensemble_gcn_layers_${gcn_layers}_weight_600
  ...
  nohup python -u train_rrcs_rdrop_t_ensemble_weight.py \
