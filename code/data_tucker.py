...
class DGLREDataloader(DataLoader):
    ...
    # h_t_limit 又是啥？=> h_entity 和 tail_entity 的对数。这里的1764 是因为 42*42 = 1764
    # 1764*97 是因为有97种关系，所以也必须带上
    def __init__(self, dataset, batch_size, shuffle=False, h_t_limit_per_batch=300, 
                    h_t_limit=1764*97, 
                    relation_num=97,max_length=512,
                    negativa_alpha=0.0, dataset_type='train'):
        super(DGLREDataloader, self).__init__(dataset,
                                             batch_size=batch_size,
                                             num_workers=10, # 初始化 DataLoader 用多线程
                                             ) 
    ...
    def __iter__(self):
        ...
        for idx, minibatch in enumerate(self.batches):
            ...
            relation_label = torch.zeros(self.batch_size, self.h_t_limit//97,dtype=torch.long).cpu()
            # self.h_t_limit 是因为
            # 不理解这里的relation_mask 的作用 => 因为同一batch中有的doc没有那么多的 relation label，所以这里有个mask操作。
            relation_mask = torch.zeros(self.batch_size, self.h_t_limit, dtype=torch.long).cpu() 
            relation_multi_label = torch.zeros(self.batch_size, self.h_t_limit).cpu() 
            relation_label.fill_(IGNORE_INDEX) # IGNORE_INDEX = -100

            max_h_t_cnt = 0 # 一个batch中各个doc实体对的最大数目 max(...)
            label_list = []
            L_vertex = []
            titles = []
            indexes = [] # TODO ?
            ...
            # ======================================== 上面的变量是存储整个batch的数据，下面的变量是当前doc 的一个临时数据
            # 对这个 batch 中的数据进行处理
            for i, example in enumerate(minibatch):
                title, entities, labels, word_id, pos_id, ner_id, mention_id, entity2mention, entity_id = \
                    example['title'], example['entities'], example['labels'], \
                ...
                cur_entity_num = len(entities) # 当前这个 doc 中的实体个数
                cur_label_num = (cur_entity_num**2)* 97  # 得到当前doc的总训练样本个数
                
                # idx2label 的值如下：{ (2, 3): [1], (11, 3): [3], (0, 1): [7] ... }
                cur_relation_label = []
                if self.dataset_type == 'train':
                    golden_label = [] # 当前 doc 的 所有正样例
                    golden_pair = list(idx2label.keys()) 
                    for item in idx2label.items():
                        pair,label = item
                        for r in label :
                            golden_label.append((pair[0],pair[1],r)) # 得到 (h_entity, t_entity,label)
                    
                    # ===TODO: 找出实体对，然后得到标签，将标签值 (待优化) ===
                    entity_pair_cnt = 0 # 表示实体对的个数 
                    for k in range(cur_entity_num):
                        for j in range(cur_entity_num):
                            flag = 0 # 标记
                            for r in range(97): # 遍历所有的关系
                                if (k,j) not in golden_pair and r == 0:
                                    cur_relation_label.append(1) # 如果真实标签没有，说明这对实体没有关系，那么在r=0这个地方就应该是1
                                    continue

                                if (k,j,r) in golden_label: # 对这个关系进标注
                                    cur_relation_label.append(1)
                                    flag = r
                                else:
                                    cur_relation_label.append(0)
                            
                            relation_label[i,entity_pair_cnt] = flag # 当前这对实体的标签是0    
                            entity_pair_cnt+=1

                    relation_multi_label[i,:cur_label_num] = torch.tensor(cur_relation_label,dtype=torch.float32)
                    # 如果是存在的关系就填充1，否则就是默认值0
                    relation_mask[i,:cur_label_num] = torch.tensor([1 for k in range(cur_label_num)],dtype=torch.float32)
                    
                    max_h_t_cnt = max(max_h_t_cnt, cur_label_num)
