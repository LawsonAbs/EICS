...
from models.RRCS_epoch import RRCS
...
def eval(stop_update_bert,opt,model, dataloader, modelname, id2rel, input_theta=-1, output=False, is_test=False, test_prefix='dev',
    ...
    for cur_i, d in tqdm(enumerate(dataloader),total=len(dataloader)):    
        with torch.no_grad():
            ...
            predictions = model.forward(words=d['context_idxs'],
                                ...
                                stop_update_bert = stop_update_bert
                                )            
    ...
    if not is_test: # 不在test
        ...
        logging(
            'max_f1 {:3.4f} | input_theta {:3.4f}, Precision {:3.4f}, Recall {:3.4f}, F1 {:3.4f} | AUC {:3.4f}' \
                .format(max_f1, input_theta, pr_y[w], pr_x[w], f1_arr[w], auc))
    ...
    logging(
        'Ignore max_f1 {:3.4f} | inhput_theta {:3.4f} dev_result P {:3.4f} dev_result R {:3.4f} dev_result F1 {:3.4f} | AUC {:3.4f}' \
            .format(f1, input_theta, pr_y[w], pr_x[w], f1_arr[w], auc))
    result = {"F1":max_f1,"precision":pr_y[w], "recall":pr_x[w], "ing f1":f1_arr[w],"input_theta":input_theta}

    return f1, auc, pr_x, pr_y ,result
