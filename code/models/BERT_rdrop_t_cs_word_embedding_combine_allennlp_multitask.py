...
5.使用多任务学习的方式，判断 指示代词指示哪个实体？ 因为我发现加入指示代词后模型效果没有提升。
...
class BERT_T_CR_MT(nn.Module):
    ''' Parameters
        config ：
    '''
    def __init__(self, config):
        super(BERT_T_CR_MT, self).__init__()
        self.config = config
    ...
    def forward(self, **params): # 对一个batch 进行操作
        ...
        batch_loss_cr = torch.tensor([0]).cuda()
        # TODO: 考虑这个操作是否可以并行执行
        ...
        for i in range(bsz): 
            ...
            if score.numel():
                batch_loss_cr = batch_loss_cr + self.bce(score,entity2pronoun[1:,1:])
            ...
            entity_representation = entity_representation_from_mention 
            batch_entity_feature[i, :entity_representation.size(0)] =  entity_representation_from_mention
        ...
        # 同时返回两个预测
        return  predictions,batch_loss_cr
