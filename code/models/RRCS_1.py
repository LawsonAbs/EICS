...
（1）对生成的图进行batch操作
'''
from multiprocessing import Pool  # 使用多进程，而不是多线程！
...
class RRCS(nn.Module):
    ...
    def forward(self, **params): # 对一个batch 进行操作
        ...
        # temp = entity_pair_multi_relation_predict.data.cpu().numpy() # 使用numpy 可能会快一些
        ...
        # print(f"****RGCN花费:{(endtime-starttime)} s....")
        ...
        # a = torch.sum(torch.isnan(h_entity_final),dim=-1) 
        # a = torch.sum(a,dim=-1)
        # a = torch.sum(a,dim=-1)
        # if a > 0:
        #     print(h_entity_final[0][0])
...
class RGCN(nn.Module):
    ...
    def forward(self, **params): # 对一个batch 进行操作
        ...
        for rgcn in self.RGCN:
            ...
            temp = torch.cat((temp,out),dim=-1)
        endtime = datetime.datetime.now()
        # print(f"****RGCN花费:{(endtime-starttime).microseconds/1000} ms")
        # 从 batch 的图中获取得到各个节点的表示，并放到 entity_bank 中
        cur_idx = 0
        for i in range(len(batch_entity_num)):  
            cur_entity_num = batch_entity_num[i]
