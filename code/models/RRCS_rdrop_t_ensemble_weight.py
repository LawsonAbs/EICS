...
'''
（1）修改RGCN得到的embedding
要不要之前的 BERT 的 embedding？ => 我觉得是不要比较好
（2）结合两者的预测值
(3) 使用RGCN 的时候结合使用边权
'''
import math
    ...
    for cur_batch_idx in range(entity_pair_multi_relation_predict.size(0)):
        ...
        for r in range(1,97): 
            ...
            if len(adj_matrix_r.T[0]) == 0:                
                ...
                edge_weight[relid2name[r]].extend(val.view(-1).tolist()) # 放上权重
        ...
        if len(data_dict) == 0:
            ...
            b = torch.sum(b,dim=-1) # 为了检查该行是否有超过阈值的数
            idx_filter = torch.nonzero(b).view(-1)
            # 使用过滤后的下标找出pair 和 a
            pred_2 = pred[idx_filter] # 单独挑选出这些满足基本条件的pred
            h_t_pairs_2= h_t_pairs[cur_batch_idx][idx_filter]  # 同时，拿出满足条件的 h_t_pairs 
            
            # step2. 接着过滤掉(0,0)这种头尾实体均相同的实体对
            x = h_t_pairs_2[:,0]
            y = h_t_pairs_2[:,1]
            mask = (x!=y)
            no_pardon_idx = mask.nonzero().view(-1) # 得到过滤后的下标            
            h_t_pairs_2 = h_t_pairs_2[no_pardon_idx] 
            pred_2 = pred_2[no_pardon_idx]

            # step3. 接着使用topk 找出前k个关系
            # 这里必须是 k = 1，是因为从每类关系中找出一个最高的即可，因为关系数太多，所以k=1就已经有很多了
            val_1 ,idx_1 = torch.topk(pred_2,k=1,dim=1) 
            # 再从上述的结果中，再找出前k个 pred
            if (len(val_1) > 0): # 如果 val_1 有值，那么就执行选取操作
                # 如果没有 topk 个，那么该怎么处理？ 那么就取 val_1.size(0) 和 5 的较小值
                val2,row_idx = torch.topk(val_1,k=min(5,val_1.size(0)),dim=0) # 实体对下标，可能没有k个怎么办？于是使用min(...)
                ...
                if len(rel_idx) > 0:
                    for i in range(len(rel_idx)):
                        ...
                        cur_val = val2[i].item()
                        data_dict[('h_type', relid2name[rel_idx[i].item()], 'h_type')].append((h_idx.view(-1),t_idx.view(-1)))
                        edge_weight[relid2name[rel_idx[i].item()]].append(cur_val) # 放上预测分值作为边权
        ...
        if len(data_dict) == 0:
            ...
            edge_weight[relid2name[r]].append(0) # 边权为0
        
        # 同时需要保证创建图的节点数也是每个doc中 entity 的个数，不能漏掉
        num_nodes_dict = {'h_type':batch_entity_num[cur_batch_idx]} # h_type 这种类型的节点的个数 
        graph = dgl.heterograph(data_dict,num_nodes_dict=num_nodes_dict)
        graph = graph.to("cuda:0") # 放到GPU中
        for i in edge_weight.items(): # 对各条边进行赋边权操作
            relation,weight =  i
            graph.edges[relation].data['w'] = torch.tensor(weight).to(pred)        
        graphs.append(graph)
...
class RelGraphConvLayer(nn.Module):
    ...
    def __init__(self,
        ...
        # 下面这个逻辑关系还不是很懂 => 就是判断是否需要使用基分解，如果不需要，就针对每个relation 定义一个weight        
        self.use_basis = num_bases < len(self.rel_names) and weight
        if self.use_weight:
            # (in_feat,out_feat)表示的就是上一层当前层的特征维度
            # 使用基分解得到整个
            if self.use_basis:
                self.weight = dglnn.WeightBasis((in_feat, out_feat), num_bases, len(self.rel_names))
            else:
                self.weight = nn.Parameter(torch.Tensor(len(self.rel_names), in_feat, out_feat).to("cuda:0"))
                nn.init.xavier_uniform_(self.weight, gain=nn.init.calculate_gain('relu'))

                # 再对self.weight 参数变成字典形式

                # 得到每种 relation 的weight
                self.weight_dict = {self.rel_names[i]: w.squeeze(0)
    ...
    def forward(self, g, inputs):
        ...
        funcs = {}
        # 之前的操作是使用这个 self.conv，但是这里简化成使用 multi_update_all 函数
        for c_etype in g.canonical_etypes:
            srctype, etype, dsttype = c_etype # 得到每条边 （左顶点，边类型，右顶点）
            # Wh = g.ndata['feat'][srctype] @ self.weight[etype]  # 节点的feature 特征 * 该边下的权重
            Wh = inputs['h_type'] @ self.weight_dict[etype]  # 节点的feature 特征 * 该边下的权重
            # 把它存在图中用来做消息传递
...
class BERT_T(nn.Module):
    ...
    def forward(self, **params): # 对一个batch 进行操作
        ...
        batch_entity_feature = torch.zeros(bsz,max_entity_num,808,dtype=torch.float32).cuda()
        ...
        for i in range(bsz): # batch是多少，就有几个图。
            ...
            batch_entity_feature[i, :select_metrix.size(0) - 1] = entity_representation[1:]
        ...
        predictions = self.predict(torch.cat(
            (h_entity, t_entity, torch.abs(h_entity - t_entity), torch.mul(h_entity, t_entity), h_entity_t,t_entity_t),dim=-1))
        
        return  predictions,batch_entity_feature,fin
...
class RRCS(nn.Module):
    ...
    def __init__(self, config):
        ...
        self.bank_size = self.gcn_dim * (self.config.gcn_layers)
        self.dropout = nn.Dropout(self.config.dropout)
        
                
        self.relid2name = get_labelid2name()
        # TODO: 之前的这个activation 是 relu，我改成了 tanh
        ...
        self.RGCN = nn.ModuleList([RelGraphConvLayer(self.gcn_dim, self.gcn_dim, self.rel_name_lists,
                                                           num_bases=config.base_num, activation=self.activation,self_loop=True, dropout=self.config.dropout)
                                         for i in range(config.gcn_layers)])

        # 图神经网络之后，紧跟几层变换做最后的预测操作
        self.predict = nn.Sequential(            
            nn.Linear( (self.gcn_dim*2+self.bank_size) * 4 , self.gcn_dim * 2),
            self.activation,
            self.dropout,
            nn.Linear(self.gcn_dim * 2, config.relation_num),
        ) 
    ...
    def forward(self, **params): # 对一个batch 进行操作
        ...
        # h_t_type = params['h_t_type'] # 得到当前batch的实体类型 => TODO: 这个要在data 中获取
        max_entity_num = torch.max(params['entity_id']) # 获取当前批次所有doc中最大的实体数
        batch_entity_num = torch.max(params['entity_id'],dim=1)[0] 
        bsz, slen = input_ids.size()
        h_t_type = torch.zeros(bsz,max_entity_num)
        entity_graphs = params['entity_graphs']
        batch_entity_feature_bert = params['batch_entity_feature']
        batch_entity_feature_transformer = params['batch_entity_feature_transformer']
        max_entity_num = torch.max(params['entity_id'])
        ...
        entity_bank = torch.zeros(bsz,max_entity_num,self.bank_size).cuda()        
        ...
        for i in range(bsz):
            ...
            cur_feature_bert = batch_entity_feature_transformer[i][0:cur_entity_num]             
            ...
            entity_bank[i,0:cur_entity_num,:] = torch.cat(temp,dim=-1) # 得到图中所有节点过完RGCN的特征
        ...
        h_entity_index = h_t_pairs[:, :, 0].unsqueeze(-1).expand(-1, -1, self.gcn_dim) 
        t_entity_index = h_t_pairs[:, :, 1].unsqueeze(-1).expand(-1, -1, self.gcn_dim)
        h_entity_bert = torch.gather(input=batch_entity_feature_bert, dim=1, index=h_entity_index)
        t_entity_bert = torch.gather(input=batch_entity_feature_bert, dim=1, index=t_entity_index)
        
                        
        h_entity_transformer = torch.gather(input=batch_entity_feature_transformer, dim=1, index=h_entity_index)
        t_entity_transformer = torch.gather(input=batch_entity_feature_transformer, dim=1, index=t_entity_index)
        ...
        h_entity_rgcn = torch.gather(input=entity_bank, dim=1, index=h_entity_index)
        t_entity_rgcn = torch.gather(input=entity_bank, dim=1, index=t_entity_index)

        
        # =========================== step 5. 预测结果 ===========================
        # predictions_rgcn = self.predict(torch.cat(
        #     (h_entity_rgcn, t_entity_rgcn, torch.abs(h_entity_rgcn - t_entity_rgcn), torch.mul(h_entity_rgcn, t_entity_rgcn)),
        #     dim=-1))
        h_entity = torch.cat([h_entity_bert,h_entity_transformer,h_entity_rgcn],dim=-1)
        t_entity = torch.cat([t_entity_bert,t_entity_transformer,t_entity_rgcn],dim=-1)
        predictions_rgcn = self.predict(torch.cat(
            (h_entity, t_entity, torch.abs(h_entity - t_entity), torch.mul(h_entity, t_entity)),
            dim=-1))
