...
class BERT_T(nn.Module):
    ...
    def __init__(self, config):
        ...
        self.predict = nn.Sequential(            
            nn.Linear(self.emb_dim * 6 + 2*768, self.emb_dim * 2),
            self.activation,
            self.dropout,
    ...
    def forward(self, **params): # 对一个batch 进行操作
        ...
        encoder_outputs = lst_hidden_states #+ input_emb # 残差连接
        # encoder_outputs = input_emb
        ...
        batch_entity_feature_raw = torch.zeros(bsz,max_entity_num,768,dtype=torch.float32).cuda() # 原始输入
        ...
        for i in range(bsz): # batch是多少，就有几个图。
            ...
            x_raw = torch.mm(select_metrix, encoder_input)  # [mention_num, bert_hid]
            x_raw = torch.cat((sentence_cls[i][:768].unsqueeze(0), x_raw), dim=0) # 拼接整个句子的表示
            ...
            entity_representation_raw = torch.mm(select_metrix, x_raw) 
            # 同理，这个select_metrix.size(0) 就是entity_num 。所以这里就相当于给一篇doc中的所有的 entity 赋特征值。 将所有的entity 属性表示放到entity_bank 中
            batch_entity_feature_raw[i, :select_metrix.size(0) - 1] = entity_representation_raw[1:]
        ...
        h_entity_raw = torch.gather(input=batch_entity_feature_raw, dim=1, index=h_entity_index[:,:,:768])
        t_entity_raw = torch.gather(input=batch_entity_feature_raw, dim=1, index=t_entity_index[:,:,:768])
        ...
        predictions = self.predict(torch.cat(
            (h_entity, t_entity, torch.abs(h_entity - t_entity), torch.mul(h_entity, t_entity), h_entity_t,t_entity_t,h_entity_raw,t_entity_raw),
            dim=-1))
