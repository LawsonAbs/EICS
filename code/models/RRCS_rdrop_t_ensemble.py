...
def create_heterograph(h_t_pairs,entity_pair_multi_relation_predict,threshold,relid2name,h_t_type,batch_entity_num):
    ...
    for cur_batch_idx in range(entity_pair_multi_relation_predict.size(0)):
        ...
        data_dict = defaultdict(list) # 这里的是list，就代表是python 在cpu中，所以对应下面有个放到GPU 的操作
        # 第r种关系下，所有的样本对的预测值
        # 关系应该从1开始，而不是0 开始
        for r in range(1,97): 
        ...
        if len(data_dict) == 0:
            ...
            if (len(val_1) > 0): # 如果 val_1 有值，那么就执行选取操作
                # 如果没有 topk 个，那么该怎么处理？ 那么就取 val_1.size(0) 和 3(超参) 的较小值
                val2,row_idx = torch.topk(val_1,k=min(3,val_1.size(0)),dim=0) # 实体对下标，可能没有k个怎么办？
        ...
        # 因为上面 data_dict 中的值是个list形式，所以这里需要将得到的graph 放到gpu中
        graph = graph.to("cuda:0") # 将图放到GPU中
        graphs.append(graph)
...
class RelGraphConvLayer(nn.Module):
    ...
    def __init__(self,
        ...
        self.conv = dglnn.HeteroGraphConv({
            ...
            # 参数说明： 这里的 in_feat 代表的就是特征输入的维度， out_feat 就是经过线性变换之后得到的维度
            #
        ...
        if self.use_weight:
            if self.use_basis: # (in_feat,out_feat)表示的就是上一层当前层的特征维度
...
class BERT_T(nn.Module):
    ...
    def __init__(self, config):
        ...
        self.predict = nn.Sequential(            
            nn.Linear(self.bank_size * 6 , self.bank_size * 2),
            self.activation,
            self.dropout,
    ...
    def forward(self, **params): # 对一个batch 进行操作
        ...
        batch_entity_feature = torch.zeros(bsz,max_entity_num,self.bank_size,dtype=torch.float32).cuda()
...
class RRCS(nn.Module):
    ...
    def __init__(self, config):
        ...
        self.gcn_in_feat = config.gcn_in_feat
        self.gcn_out_feat = config.gcn_out_feat
        self.bank_size = self.gcn_out_feat * (self.config.gcn_layers)
        self.dropout = nn.Dropout(self.config.dropout)
        ...
        # TODO: 因为有个维度的转换，所以这里还是有问题
        # 如果前后维度不一致该怎么解决？ => 之前这里是用for循环，这里我直接写出来
        self.RGCN = nn.ModuleList([RelGraphConvLayer(self.gcn_in_feat, self.gcn_out_feat, self.rel_name_lists,
                                                    num_bases=config.base_num, activation=self.activation,self_loop=True, dropout=self.config.dropout),
                                   RelGraphConvLayer(self.gcn_out_feat, self.gcn_out_feat, self.rel_name_lists,
                                                    num_bases=config.base_num, activation=self.activation,self_loop=True, dropout=self.config.dropout)])

        # 图神经网络之后，紧跟几层变换做最后的预测操作
        self.predict = nn.Sequential(
            # self.gcn_dim * 2 表示有2个
