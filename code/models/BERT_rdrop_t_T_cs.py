'''
思想：我们认为实体对之间是应该有关系的，而之前的工作仅仅考虑实体之间的交互是不够的。
实现：将得到的实体组成对，串成一串，放入到transformer 中，这个有点儿像使用GCN来对各个entity pair 之间交互
...
class BERT_T(nn.Module):
    ...
    def __init__(self, config):
        ...
        # 用于实体间的交互
        self.encoder_entity = BertEncoder(config=BertConfig(hidden_size=self.emb_dim,
        ...
        # TODO: 这里可以考虑使用一个双线性层
        self.project = nn.Linear(1648,768)
        self.layernorm = nn.LayerNorm(768)
        # 用于实体对的交互
        self.encoder_entity_pair = BertEncoder(config=BertConfig(hidden_size=768,
                                                            num_attention_heads=12, 
                                                            num_hidden_layers = config.pair_interaction_layers,
                                                            hidden_dropout_prob = config.interaction_hidden_dropout_prob
                                                            ))
        ...
        self.predict = nn.Sequential(            
            nn.Linear(self.emb_dim * 6 + 768 , self.emb_dim * 2),
            self.activation,
            self.dropout,
    ...
    def forward(self, **params): # 对一个batch 进行操作
        ...
        batch_entity_feature = torch.zeros(bsz,max_entity_num,self.emb_dim,dtype=torch.float32).cuda()
        ...
        # 实体对的mask，因为各个doc中的实体数不一致
        entity_pair_mask = torch.zeros(bsz,512)
        
        # 实体对的表示，这里直接采用拼接的方式来获取
        batch_entity_pair_feature = torch.zeros(bsz,512,768,dtype=torch.float32).cuda()
        ...
        for i in range(bsz): # 对每个batch
            ...
            # TODO: 23*22= 506 ，所以至多只支持23个实体。但是超出的实体该怎么办呢？
            entity_pair_num = min(512,cur_id_num*cur_id_num)
            entity_pair_mask[i,:entity_pair_num] = 1

            # 获取每篇doc的实体 => 组合得到 entity pair embedding
            cur_doc_entity = batch_entity_feature[i,:cur_id_num]
            row = cur_doc_entity.repeat(cur_doc_entity.size(0),1)
            col = cur_doc_entity.repeat(1,cur_doc_entity.size(0)).view(-1,cur_doc_entity.size(1))            
            # 将row和 col 拼接，并且放入到 batch_entity_pair_feature 中   # TODO: 要不要将这个额外的type 等信息放到其中？
            cur_doc_entity_pair_feature = torch.cat((row,col),dim=-1)
            
            # 将这两个维度压缩成 768, 方便后面做计算             
            batch_entity_pair_feature[i,:entity_pair_num] =  self.layernorm(self.project(cur_doc_entity_pair_feature)[:entity_pair_num])
        ...
        out = self.encoder_entity(batch_entity_feature,attention_mask = extended_attention_mask)
        fin_t = out[0] # 得到最后的表示
                       
        extended_pair_attention_mask = get_extended_attention_mask(attention_mask = entity_pair_mask,input_shape=entity_pair_mask.size()).to(torch.device('cuda:0'))

        # =========================== step 4. 组成 entity pair，然后送入到 transformer 中进行交互==========================
        entity_pair_out = self.encoder_entity_pair(batch_entity_pair_feature,attention_mask = extended_pair_attention_mask)[0]
        b = torch.zeros(bsz,1,768).cuda() # 初始化为0向量
        entity_pair_out = torch.cat((entity_pair_out,b),dim=1)
        ...
        a_idx = h_t_pairs[:,:,0]
        b_idx = h_t_pairs[:,:,1]
        c = torch.max(entity_id,dim=-1)[0].unsqueeze_(1).expand(-1,h_t_pairs.size(1))
        entity_pair_idx = (a_idx * c + b_idx).unsqueeze(-1).expand(-1,-1,768)
        entity_pair_idx = torch.where(entity_pair_idx<512,entity_pair_idx,torch.tensor(512).cuda())
        # 接着，从上面的输出中，获取对应下标即可
        pair_emb_t = torch.gather(input=entity_pair_out,dim=1,index=entity_pair_idx)
        ...
        h_entity_t = torch.gather(input=fin_t,dim=1,index=h_entity_index)
        t_entity_t = torch.gather(input=fin_t,dim=1,index=t_entity_index)
        ...
        predictions = self.predict(torch.cat(
            (h_entity, t_entity, torch.abs(h_entity - t_entity), torch.mul(h_entity, t_entity), h_entity_t,t_entity_t,pair_emb_t),
            dim=-1))
