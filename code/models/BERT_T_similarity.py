...
2.使用 torch.cosine_similarity 计算各个表示之间的相似性，并且让这些表示互相远离
torch.cosine_similarity(self.predict[3].weight[91],self.predict[3].weight[3],dim=0) 
学习的目标就是让其损失变小。
'''
import math
import inspect
...
class BERT_T(nn.Module):
    ...
    def calculate_similarity(self):
        ...
        sim_loss = torch.sum(torch.where(upper_matrix > 0,upper_matrix,z))         
        return sim_loss
