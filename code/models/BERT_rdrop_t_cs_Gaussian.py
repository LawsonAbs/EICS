'''
1.使用原始的BertEncoder，BertModel 
2.在Attention过程中融入 Normal Distribution 信息
'''
import math
import inspect
import datetime
...
class BertEmbeddings(nn.Module):
    ...
    def __init__(self, config):
        ...
        # 控制大小写
        self.upper_embeddings = nn.Embedding(num_embeddings=1,embedding_dim=config.hidden_size)

    def forward(
        self, input_ids=None, token_type_ids=None, position_ids=None, inputs_embeds=None, past_key_values_length=0,capital_mask = None
    ):
        ...
        embeddings = inputs_embeds + token_type_embeddings #+ self.upper_embeddings(torch.tensor([0],dtype=torch.long))
        if self.position_embedding_type == "absolute":
            position_embeddings = self.position_embeddings(position_ids)
            embeddings += position_embeddings

        if capital_mask is not None:
            tmp = self.upper_embeddings(torch.tensor([0],dtype=torch.int).cuda()) # 得到单位的向量
            tmp = tmp.expand(capital_mask.size(0),capital_mask.size(1),-1)
            capital_mask = capital_mask.unsqueeze(2).expand(-1,-1,tmp.size(2))
            
            capital_info = torch.where(capital_mask>0,tmp,capital_mask)
            
            embeddings = embeddings + capital_info
...
class BertSelfAttention(nn.Module):
    def __init__(self, config):
        ...
        d = 512
        a = torch.tensor([i for i  in range(d)])
        a = a.unsqueeze(1)
        a = a.expand(-1,a.size(0))
        
        b = torch.tensor([i for i in range(d)])
        b = b.unsqueeze(0)        
        b = b.expand(b.size(1),-1)        
        self.relative_position = abs(a-b)        
        self.alpha = config.alpha

        res = list(map(self.f,torch.flatten(self.relative_position)))        
        # 这个先验有个问题就是： 会导致对自己的attention值最大这是不合理的
        # 使用正态分布作为先验知识
        # 这个参数也是要梯度更新的
        self.prior_normal_distribution = torch.tensor(res,dtype=torch.float64,requires_grad=True).reshape(d,d).cuda()
        
        
    def f(self,d):
            # return float(math.exp(-self.alpha * (d * d )))
            return float(-abs(self.alpha * (d * d ))-0.2) # 
    ...
    def forward(
        ...
        # 在这里加入正态分布值
        attention_scores += self.prior_normal_distribution[:attention_scores.size(-1),:attention_scores.size(-1)]
...
class BertAttention(nn.Module):
    ...
    def forward(
        ...
        self_outputs = self.self( # self attention output
            hidden_states,
            attention_mask,
            head_mask,
...
class BertModel(BertPreTrainedModel):
    ...
    """
    # alpha 是用于传给 attention 的参数
    def __init__(self, config, add_pooling_layer=True,alpha=0.0002):
...
class BERT_T(nn.Module):
    ...
    def __init__(self, config):
        ...
        # TODO: 学习一下 **kwargs的使用。这里alpha本不是参数，但是最后却用上了
    ...
    def forward(self, **params): # 对一个batch 进行操作
        ...
        mask = params['mask'] 
        # capital_mask = params['capital_mask']
        ...
        # 先过bert.
        # 传入原字符串的大小写信息
        # output = self.bert(input_ids=input_ids, attention_mask=mask,capital_mask = capital_mask)
        ...
        # encoder_outputs 是 last_hidden_states  和 之前权重的相加值
        ...
        # 将Bert得到的结果进行归一化操作
        # h_entity = F.normalize(h_entity,dim=-1)
        # t_entity = F.normalize(t_entity,dim=-1)
        # h_entity = F.normalize(h_entity_t,dim=-1)
        # t_entity_t = F.normalize(t_entity_t,dim=-1)
        ...
        return  predictions,batch_feature_bert
