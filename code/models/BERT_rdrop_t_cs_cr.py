...
from transformers import BertModel,BertTokenizer,BertConfig,RobertaModel
...
class BERT_T(nn.Module):
    ...
    def __init__(self, config):
        ...
        # 向量初始维度的dim，也就是bert/roberta后的维度
        self.emb_dim = config.bert_hid_size
        ...
        if config.bert_path == "bert-base-uncased":
            ...
            self.bert = RobertaModel.from_pretrained(pretrained_model_name_or_path=config.bert_path,
                                                    attention_probs_dropout_prob=config.bert_dropout,
                                                    hidden_dropout_prob=config.bert_dropout,  
                                                    output_hidden_states=True # 输出每一层的 hidden states
            )
        ...
        self.cr_score_way = "bilinear"
        # 定义几个参数用于控制计算attention 
        self.W_additive = nn.Linear(in_features=self.emb_dim,out_features=self.emb_dim) # additive score 中的W
        self.U_additive = nn.Linear(in_features=self.emb_dim,out_features=self.emb_dim) # additive score 中的U
        
        self.W_bilinear = nn.Linear(in_features=self.emb_dim,out_features=self.emb_dim,bias=False) # bilinear score 中的W
    ...
    def forward(self, **params): # 对一个batch 进行操作
        ...
        entity_coref_masks = params['entity_coref_mask']
        pronoun_ids = params['pronoun_id'] # 获取每篇doc的代词信息
        ...
        for i in range(bsz): 
            ...
            # 通过合并mention的特征就能获取到 entity_representation_NER
            entity_representation_NER = torch.mm(select_metrix, x) 
            
            
            # 根据pronoun_id 获取每个代词的embedding。 因为各个pronoun 都是只有一个字长，所以直接选择index即可
            pronoun_index = pronoun_ids[i].nonzero().view(-1) # 得到非0位置的id，获取这些id对应的 embedding
            pronoun_embedding = encoder_output[pronoun_index] # 得到代词的embedding 
            # 因为可能有的指示代词超出了512限制，所以这里得截断一下
            cur_entity_coref_mask = entity_coref_masks[i][:,:pronoun_index.size(0)] # 得到当前的实体指代信息
            
            # 计算score值
            score = self.score(pronoun_embedding,entity_representation_NER[1:],cur_entity_coref_mask,score_way=self.cr_score_way)
            
            # q = entity_representation_NER[1:]
            # a = (self.W_bilinear(pronoun_embedding) @ q.T).T
            # similarity = a * (1-cur_entity_coref_mask) + cur_entity_coref_mask * (-1e30)
            # score = torch.softmax(similarity,dim=0)
            if not self.training:                                        
                batch_cr_scores.append(score) 
                # print("score",score)
            # # TODO: 但其实并非所有的 指示代词都是有意义的，所以怎么整？
            # 根据得到的score信息，写入到entity_representation_CR中，带有指示代词的 实体信息
            # 还需要根据分母的信息进行一个加权会比较好
            entity_representation_CR = (entity_representation_NER[1:] + score @ pronoun_embedding) / (1+torch.sum(score,dim=-1,keepdim=True))

            # 给所有的实体赋值 embedding 
            # 同理，这个select_metrix.size(0) 就是entity_num 。所以这里就相当于给一篇doc中的所有的 entity 赋特征值。 将所有的entity 属性表示放到entity_bank 中
            batch_entity_feateature[i, :select_metrix.size(0) - 1] = entity_representation_CR

        # =========================== step 3. 使用n层自定义的Transformer 框架 ===========================
        # 对获取到的entity_features 送到n层transformer 中进行处理
        ...
        return  predictions,batch_cr_scores

    # 使用不同的方式计算相似度分数
    # q是查询向量。在本例子中x是 pronoun embedding，q是 entity embedding
    def score(self,x,q,entity_coref_mask,score_way):
        if score_way == 'cosine':
            return self._cosine_score(x,q,entity_coref_mask)
        elif score_way == 'additive':
            return self._additive_score(x,q,entity_coref_mask)
        elif score_way == 'scale_dot':
            return self._scale_dot_score(x,q,entity_coref_mask)        
        elif score_way == "bilinear":
            return self._bilinear_score(x,q,entity_coref_mask)
        else:
            # TODO: 
            raise Exception()

    # 直接使用cosine 相似度计算
    def _cosine_score(self,x,q,entity_coref_mask):
        denominator = torch.sqrt(torch.sum(q**2,dim=-1,keepdim=True)) * torch.sqrt(torch.sum((x.T)**2,dim=0,keepdim=True))
        similarity = (q @ x.T)/denominator # 计算二者的相似度
        similarity = similarity * (1-entity_coref_mask) + entity_coref_mask * (-1e30)
        score = torch.softmax(similarity,dim=0)
        return score
        
    
    # 使用加性attention
    def _additive_score(self,x,q,entity_coref_mask):
        torch.tanh(self.W_additive @ x  + self.U_additive @ q)
        pass

    # 使用 scale-dot attention 打分
    def _scale_dot_score(self,x,q,entity_coref_mask):
        pass


    # 使用双线性 打分
    def _bilinear_score(self,x,q,entity_coref_mask):
        a = (self.W_bilinear(x) @ q.T).T
        similarity = a * (1-entity_coref_mask) + entity_coref_mask * (-1e30)
        score = torch.softmax(similarity,dim=0)
        return score
