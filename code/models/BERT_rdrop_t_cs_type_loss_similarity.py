...
class BERT_T(nn.Module):
    ...
    def calculate_similarity(self):
        ...
        # 计算方式1：使用 mse 计算损失(先过滤掉小于0.05的不需要的结果，再计算mse值)
        rel_upper_matrix = torch.where(abs(upper_matrix) > 0.05,upper_matrix,z) # 如果超过 0.05，则计算损失，否则不计算
        sim_loss_mse = torch.sum((rel_upper_matrix)**2) # MSE
        
        # 计算方式2：直接返回相似度值作为损失（如果平方之后损失太小（只有超过0.01的才会被优化），则不考虑该损失）
        sim_loss_sum = upper_matrix**2
        sim_loss_sum  = torch.sum(torch.where(sim_loss_sum > 0.01,sim_loss_sum,z))
        return sim_loss_sum
