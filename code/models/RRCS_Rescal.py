    ...
    for cur_batch_idx in range(entity_pair_multi_relation_predict.size(0)):
        ...
        for r in range(97):
            ...
            if len(idx) == 0:  # 要随机选一对实体处理
                ...
                continue
            adj_matric_r = h_t_pairs[cur_batch_idx][idx]
            data_dict[('h_type', relid2name[r], 'h_type')] = (adj_matric_r.T[0].view(-1),adj_matric_r.T[1].view(-1))
...
class RRCS(nn.Module):
    ...
    def __init__(self, config):
        ...
        # 但是在RRCS中，我们使用97种 relation
        self.rel_name_lists = list(self.relid2name.values()) # 强转成list
        self.RGCN = nn.ModuleList([RelGraphConvLayer(self.gcn_dim, self.gcn_dim-12, self.rel_name_lists,
        ...
        # 定义一个关系矩阵                
        self.R = torch.tensor(np.random.uniform(-1, 1, (config.relation_num,self.gcn_dim,self.gcn_dim)),
                                    dtype=torch.float,device='cuda')                
        
        self.threshold = config.graph_threshold

        # 下面三个参数是供 rescal 使用的
        self.block_size = 101
        self.bilinear = nn.Linear(808 * self.block_size, config.relation_num)
        

        # 过完Bert的预测，得到各个entity pair之间的 multi_relation。
        self.predict = nn.Sequential(            
            nn.Linear(2400*4, self.gcn_dim ),
    ...
    def forward(self, **params): # 对一个batch 进行操作
        ...
        b1 = h_entity.view(bsz,-1, 808 // self.block_size, self.block_size)
        b2 = t_entity.view(bsz,-1, 808 // self.block_size, self.block_size)
        bl = (b1.unsqueeze(4) * b2.unsqueeze(3)).view(bsz,-1, 808 * self.block_size)
        entity_pair_multi_relation_predict = self.bilinear(bl)
            
        
        # =========================== step 5. 将得到的entity 表示 + entity relation 建图(relation entity graph) ===========================
        ...
        entity_graphs = create_heterograph(h_t_pairs,
                                        entity_pair_multi_relation_predict,
                                        self.threshold,
                                        self.relid2name,
                                        h_t_type,
                                        batch_entity_num.tolist()
                                        )

        # =========================== step 5. 对图执行GCN操作  ===========================
        # 在图上应用RGCN
        # Bert 最后一层 + RGCN的两层输出        
        entity_bank = torch.zeros(bsz,max_entity_num,2400).cuda()
        # 对每个graph（doc）做RGCN操作，然后得到他们的返回值，将其做为每篇doc中的 entity 的最终表示
        batch_feature_bert = torch.cat(batch_feature_bert,dim=0)
        assert batch_feature_bert.size(0) == entity_graphs.num_nodes()
        
        # 保证特征数（实体节点数）和图节点数相同 => 如果上面的 create_heterograph 的节点数不够，则容易出问题            
        # 缓存所有节点的特征
        temp = batch_feature_bert
        entity_graphs = entity_graphs.to(batch_feature_bert.device)
        for rgcn in self.RGCN:
            # 输入的特征是当前这个图的entity的特征
            out = rgcn(entity_graphs,{"h_type":batch_feature_bert})['h_type'] # rgcn on entity_graphs[i]
            temp = torch.cat((temp,out),dim=-1)
            
        # 从 batch 的图中获取得到各个节点的表示，并放到 entity_bank 中
        cur_idx = 0
        for i in range(len(batch_entity_num)):  
            cur_entity_num = batch_entity_num[i]
            next_idx = cur_idx + cur_entity_num
            entity_bank[i,0:cur_entity_num,:] = temp[cur_idx:next_idx] # 得到图中所有节点的特征
            cur_idx = next_idx
        ...
        return  predictions_rgcn,entity_pair_multi_relation_predict