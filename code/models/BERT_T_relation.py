...
'''
01.使用n层 transformer 对最后得到的entity 进行建模
02.使用训练好的relation的embedding建模，而不是随机初始化
'''
import math
import inspect
...
from get_relation_definition import get_relation_emb
...
class BERT_T(nn.Module):
    ...
    def __init__(self, config):
        ...
        self.predict = nn.Sequential(            
            nn.Linear(self.bank_size * 7 , 768*2),
            self.activation,
            self.dropout,
            nn.Linear(768*2 , config.relation_num), 
        )
        # 定义使用的attention操作  
        ...
        # 获取关系向量的表示。第1类（country）到第96类
        # 转换成tensor
        self.relation_emb = torch.tensor(get_relation_emb())
        
        # 随机初始化得到 na 类的关系表示
        self.na_emb = torch.empty(1,768*2,requires_grad=True)
        bound = 1 / math.sqrt(768*2) if self.bank_size > 0 else 0
        init.uniform_(self.na_emb, -bound, bound)

        # 将realtion_emb 归一化到指定区间 [-bound,bound]
        # 因为每行值有不同的范围，所以需要针对每行计算
        # k = (2*bound)/torch.max(self.relation_emb) - torch.min(self.realtion_emb) # 得到放缩系数
        # 先简单的直接Normalize
        self.relation_emb = normalize(self.relation_emb,p=2,dim=-1)
        

        # 将所有关系拼接到一起，并设置成可训练的 parameter
        self.relation_emb = Parameter(torch.cat((self.na_emb,self.relation_emb),dim=0).T)
        
        self.bias = Parameter(torch.empty(97,requires_grad=True))
        bound = 1 / math.sqrt(768*2) if self.bank_size > 0 else 0
    ...
    def forward(self, **params): # 对一个batch 进行操作
        ...
        # predictions = predictions @ self.relation_emb + self.bias
        return predictions
