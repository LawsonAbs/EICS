...
class RRCS(nn.Module):
    ...
    def forward(self, **params): # 对一个batch 进行操作
        ...
        # TODO:这里面的值的波动也太大了？torch.max() = 124, torch.min() = -172. 是为什么？
        ...
        #     rel_map = set([id2name[j[2]] for j in labels[i]]) # 找出当前关系的集合
        #     ids = input_ids[i,0:100]
        #     res = tokenizer.convert_ids_to_tokens(ids)
        #     print(f"当前文档对应的图的边数是：{entity_graphs[i].num_edges()}，边的类型有：{entity_graphs[i].etypes}，顶点数是：{entity_graphs[i].num_nodes()}，当前文档的关系有：{len(labels[i])}条，关系类型是：{rel_map}，当前文档内容是：{res}")
