...
2021/12/28
（1）生成的图不再batch，分开操作 => 速度会慢很多！
(2) 在某个 epoch 之后，再进行RGCN的训练
'''
import random
...
def create_heterograph(h_t_pairs,entity_pair_multi_relation_predict,threshold,relid2name,h_t_type,batch_entity_num):    
    ...
    for cur_batch_idx in range(entity_pair_multi_relation_predict.size(0)):
        ...
        for r in range(1,97): 
            ...
            # 如果没有这种关系的实体对，那么就直接使用第一对实体对中的头尾实体
            # 如果没有这种关系的实体对，那么就直接使用可能性最大的一堆实体
            # 如果没有这种关系的实体对，那么就下一个
...
class RRCS(nn.Module):
    ...
    def __init__(self, config):
        ...
        self.epoch_threshold = config.epoch_threshold 
    ...
    def forward(self, **params): # 对一个batch 进行操作
        ...
        if cur_epoch <= self.epoch_threshold:
