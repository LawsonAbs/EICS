...
class BERT_T(nn.Module):
    ...
    def forward(self, **params): # 对一个batch 进行操作
        ...
        for i in range(bsz): # batch是多少，就有几个图。
            ...
            # x_raw = torch.mm(select_matrix, encoder_input)  # [mention_num, bert_hid]
            # x_raw = torch.cat((sentence_cls[i].unsqueeze(0), x_raw), dim=0) # 拼接整个句子的表示
            ...
            select_matrix = entity2mention_table[i].float()  # [local_entity_num, mention_num+1]
            select_matrix[0][0] = 1 
            ...
            entity_representation = torch.mm(select_matrix, x) # 通过合并mention的特征就能获取到 entity_representation
            # 同理，这个select_matrix.size(0) 就是entity_num 。所以这里就相当于给一篇doc中的所有的 entity 赋特征值。 将所有的entity 属性表示放到entity_bank 中
            batch_entity_feature[i, :select_matrix.size(0) - 1] = entity_representation[1:]
            

            # entity_representation_raw = torch.mm(select_matrix, x_raw) 
            # 同理，这个select_matrix.size(0) 就是entity_num 。所以这里就相当于给一篇doc中的所有的 entity 赋特征值。 将所有的entity 属性表示放到entity_bank 中
            # batch_entity_feature_raw[i, :select_matrix.size(0) - 1] = entity_representation_raw[1:]
