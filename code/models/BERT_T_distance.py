...
from .modeling_bert import BertModel,BertConfig
...
class BERT_T(nn.Module):
    ...
    def forward(self, **params): # 对一个batch 进行操作
        ...
        mask = params['mask']
        length_mask = params['length_mask']
        bsz, slen = input_ids.size()
        # =========================== step 1. 获取原始句子、mention、entity 的表示  ===========================
        # GPU
        # 先过bert.  encoder_outputs 是 last_hidden_states  
        output = self.bert(input_ids=input_ids, attention_mask=mask, length_mask=length_mask)
