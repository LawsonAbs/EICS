'''
1.使用n层 transformer 对最后得到的entity 进行建模
2.使用logsumexp 对mention进行合并操作
'''
import math
import inspect
...
class BERT_T(nn.Module):
    ...
    def forward(self, **params): # 对一个batch 进行操作
        ...
        max_entity_num = torch.max(params['entity_id']) # 选择当前batch 中最大数目的entity 作为 entity_num （在dev_44.json 中，其值为31）
        ...
        entity_bank = torch.zeros(bsz, max_entity_num, self.bank_size).to(torch.device('cuda:0')) 
        ...
        for i in range(bsz):  # 对每个图都进行一次操作
            ...
            # 使用 logsumexp 从mention中获取各个实体的表示
            cur_entity2mention_table = entity2mention_table[i].float()
            mention_num = cur_entity2mention_table.size(1) - 1
            entity_num = cur_entity2mention_table.size(0) - 1
            cur_doc_entity_emb = []
            cur_doc_mention_emb = features[cur_idx:cur_idx + mention_num + 1]
            for j in range(1,entity_num+1): # 1,entity_num+1
                mention_idx = cur_entity2mention_table[j].nonzero().view(-1) # 找出entity对应mention 的位置
                if len(mention_idx) == 0 : # 如果这个实体对应的mention_idx 为空
                    # 1.最开始处理为 CLS 向量，但是效果并不好
                    # 2.接着处理为 0向量
                    # mention_idx = 0 
                    cur_entity_emb = torch.zeros(808).to(cur_doc_mention_emb)
                else:    
                    cur_mention_emb = cur_doc_mention_emb[[mention_idx]]
                cur_entity_emb = torch.logsumexp(cur_mention_emb,dim=0) # 得到对应的entity 表示
                # cur_entity_emb = cur_mention_emb[0] # 得到对应的entity 表示
                cur_doc_entity_emb.append(cur_entity_emb)
            cur_doc_entity_emb = torch.stack(cur_doc_entity_emb)
            entity_bank[i, :entity_num ] = cur_doc_entity_emb 
            global_info[i] = features[cur_idx]
            cur_idx += (mention_num+1)
        
        # =========================== step 3. 使用 n 层自定义的Transformer 框架 ===========================                
        # 对获取到的entity_features 送到 n 层transformer 中进行处理
        ...
        predictions = self.predict(torch.cat(
            (h_entity, t_entity, torch.abs(h_entity - t_entity), torch.mul(h_entity, t_entity), global_info,h_entity_t,t_entity_t),
            dim=-1))
